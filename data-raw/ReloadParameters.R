...
exampleobs = data.frame(SWC = S1$Soil$W.1*(soil_thetaFC(examplesoil1)[1]), 
             ...
             E_T1_157 = S1$Plants$Transpiration[,"T1_157"]/x1$above["T1_157","LAI_expanded"],
             E_T2_176 = S1$Plants$Transpiration[,"T2_176"]/x1$above["T2_176","LAI_expanded"],
             FMC_T1_157 = fmc[,"T1_157"],
             FMC_T2_176 = fmc[,"T2_176"],
             BAI_T1_157 = S1$PlantStructure$SapwoodArea[,"T1_157"]*S1$PlantGrowth$SAgrowth[,"T1_157"],
             BAI_T2_176 = S1$PlantStructure$SapwoodArea[,"T2_176"]*S1$PlantGrowth$SAgrowth[,"T2_176"])
#Add normal error
...
exampleobs$E_T1_157 = exampleobs$E_T1_157 + rnorm(nrow(exampleobs), mean = 0, sd = sd(exampleobs$E_T1_157)/4)
exampleobs$E_T2_176 = exampleobs$E_T2_176 + rnorm(nrow(exampleobs), mean = 0, sd = sd(exampleobs$E_T2_176)/4)
exampleobs$FMC_T1_157 = exampleobs$FMC_T1_157 + rnorm(nrow(exampleobs), mean = 0, sd = sd(exampleobs$FMC_T1_157)/4)
exampleobs$FMC_T2_176 = exampleobs$FMC_T2_176 + rnorm(nrow(exampleobs), mean = 0, sd = sd(exampleobs$FMC_T2_176)/4)
exampleobs$BAI_T1_157 = exampleobs$BAI_T1_157 + rnorm(nrow(exampleobs), mean = 0, sd = sd(exampleobs$BAI_T1_157)/4)
exampleobs$BAI_T2_176 = exampleobs$BAI_T2_176 + rnorm(nrow(exampleobs), mean = 0, sd = sd(exampleobs$BAI_T2_176)/4)
