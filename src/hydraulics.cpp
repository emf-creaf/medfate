...
List E2psiBelowground(double E, NumericVector psiSoil, 
  ...
  return(List::create(Named("E") = E, Named("ERhizo")=Erhizo, Named("psiRhizo") = psiRhizo, Named("psiRoot") = psiRoot, Named("x") = x));
} 
...
List E2psiXylemCapacitance(double E, double psiRootCrown, 
  ...
  for(int i=0;i<n;i++) {
    ...
    psiPLC = apoplasticWaterPotential(1.0-PLC[i], c, d);
...
List E2psiAboveground(double E, double psiRootCrown, 
  ...
  for(int i=0;i<nStemSegments; i++) {
    ...
    psiStem[i] = E2psiXylem(E, psiUp, kxsegmax, stemc, stemd, psiPLCStem[i], psiStep, psiMax); //Apliquem la fatiga per cavitacio a la caiguda de potencial a la tija 
    psiUp = psiStem[i];
  }  
  double psiLeaf = E2psiXylem(E, psiStem[nStemSegments-1], kleafmax, leafc, leafd, 0.0, psiStep, psiMax); 
...
List E2psiAbovegroundCapacitanceDisconnected(double E,                           
  ...
  //Make copy of initial vectors
  PLCstem = clone(PLCstem);
  RWCsympstem = clone(RWCsympstem);
  ...
  double psiLeafSymp = symplasticWaterPotential(RWCsympleaf, leafpi0, leafeps);
  for(int i=0;i<n;i++) {
    VStem[i] = Vsegmax*stemfapo*(1.0-PLCstem[i]);
    psiPLCStem[i]=  apoplasticWaterPotential(1.0-PLCstem[i], stemc, stemd);
    psiStorageStem[i] = symplasticWaterPotential(RWCsympstem[i], stempi0, stemeps);
    // Rcout<< VStem[i] << " "<<psiPLCStem[i] << " "<<psiStorageStem[i] <<"\n";
  }
  ...
  for(int s=0;s<nSubSteps;s++) {
    ...
    double FverStemLeaf = xylemConductance(psiLeaf, kleafmax, leafc, leafd)*(psiPLCStem[n-1]-psiLeaf);
    // Rcout<< "Vertical flow stem to leaf "<<FverStemLeaf<<"\n";
    //Calculate lateral symplastic flow (positive when symplasm has less negative WP)
    double FlatLeaf = klat*(psiLeafSymp - psiLeaf);
    // Rcout<< "Lateral flow to leaf symplasm "<<FlatLeaf<<"\n";
    ...
    for(int i=0;i<n;i++) {
      //Lateral flow
      FlatStem[i] =  klat*(psiStorageStem[i]-psiPLCStem[i]);
      
      //Towards above
      if(i<(n-1)) {
        Fverapo2[i] = xylemConductance(psiPLCStem[i], kxsegmax, stemc, stemd)*(psiPLCStem[i]-psiPLCStem[i+1]);
      ...
      //From below
      if(i>0) {
        Fverapo1[i] = xylemConductance(psiPLCStem[i-1], kxsegmax, stemc, stemd)*(psiPLCStem[i-1]-psiPLCStem[i]);
    ...
    if(Vleafssub > (Vleaf*leaffapo)) { //Add output flow to close balance
      ...
      Esubfin = Esubfin + dif*(m3tommol/tstepsub);
    }
    Efin += Esubfin; 
    // Rcout<<newRVLeaf;
    ...
    for(int i=0;i<n;i++) {
      ...
      // VStem[i] = std::max(0.0,std::min(VStem[i],Vsegmax*stemfapo));
      PLCstem[i] = 1.0- (VStem[i]/(Vsegmax*stemfapo));
      psiPLCStem[i]=  apoplasticWaterPotential(1.0-PLCstem[i], stemc, stemd);
      //Symplastic compartment
      RWCsympstem[i] = (Vsegmax*(1.0-stemfapo)*RWCsympstem[i] + (tstepsub/m3tommol)*(Fversym1[i] - Fversym2[i] - FlatStem[i]))/(Vsegmax*(1.0-stemfapo));
      psiStorageStem[i] = symplasticWaterPotential(RWCsympstem[i], stempi0, stemeps);
...
List E2psiNetwork(double E, NumericVector psiSoil, 
  ...
  if(!NumericVector::is_na(psiRoot)) {
    List E2psiAG = E2psiAboveground(E, psiRoot, 
                                    kstemmax, stemc, stemd,
  ...
  return(List::create(Named("E") = E, Named("ERhizo")=ERhizo, Named("psiRhizo") = psiRhizo, 
                      Named("psiRoot") = psiRoot, Named("psiStem") = psiStem, Named("psiLeaf") = psiLeaf, 
...
List supplyFunctionBelowground(NumericVector psiSoil, 
  ...
  List sol = E2psiBelowground(minFlow, psiSoil, 
                          ...
                          psiStep, psiMax, ntrial,psiTol, ETol);
  supplyElayers(0,_) = sol["ERhizo"];
  supplyPsiRhizo(0,_) = sol["psiRhizo"];
  supplyE[0] = sol["E"];
  ...
  for(int i=1;i<maxNsteps;i++) {
    ...
    sol = E2psiBelowground(supplyE[i], psiSoil, 
                       ...
                       sol["x"],
                       psiStep, psiMax, ntrial,psiTol, ETol);
    supplyElayers(i,_) =  sol["ERhizo"];
    supplyPsiRhizo(i,_) = sol["psiRhizo"];
...
/**
 * Calculate capacity of stem per leaf area (in m3·m-2)
 * 
 * Al2As - Leaf area to sapwood area ratio (in m2·m-2)
 * height - plant height (in cm)
...
/**
 * Calculate  capacity of leaves per leaf area (in m3·m-2)
 * 
