...
double windAdjustmentFactor_c(double topShrubHeight, double bottomCanopyHeight, double topCanopyHeight, double canopyCover){
  ...
  if(std::isnan(topCanopyHeight)) crownFillProportion=0.0;
  double WAF;
  //unsheltered vs sheltered (5% of crownFillProportion)
  if(crownFillProportion<0.05) WAF = unshelteredMidflameWindSpeed_c(10.0, topShrubHeight)/10.0;
  else WAF = shelteredMidflameWindSpeed_c(10.0, crownFillProportion, topCanopyHeight)/10.0;
  return(WAF);
}
...
double windSpeedAtHeightOverCanopy_c(double z, double wind20H, double canopyHeight) {
  ...
  double Uh = windSpeedAtCanopyHeight_c(wind20H, canopyHeight);
 ...
 */
double windSpeedLalicExtinction_c(double z, double wind20H, double LAIc, double canopyHeight, double crownBaseHeight) {
  double Uh = windSpeedAtCanopyHeight_c(wind20H, canopyHeight);
