...
#'       \item{\code{leafCavitationEffects, stemCavitationEffects [= TRUE/TRUE]}: A flag indicating whether cavitation effects on conductance of leaf apoplasm and stem are applied. Only relevant for \code{transpirationMode = "Sperry"}.}
...
#'           \itemize{
#'             \item{\code{kPAR [= 0.5]}: Mistletoe light extinction coefficient.}
#'             \item{\code{g [= 0.8]}: Mistletoe water storage capacity per LAI unit.}
...
#'       \item{\code{leafConductanceEstimation [= TRUE]}: Flag to indicate that leaf maximum conductance should be estimated as a fraction of resistance depending on height instead of using measured \code{VCleaf_kmax} values.}
#'       \item{\code{rootConductivityEstimation [= TRUE]}: Flag to indicate that root maximum conductivity should be estimated from stem conductivity instead of using measured \code{Kmax_rootxylem} values.}
...
#'       \item{\code{Q10_1_gmin [= 1.2]}: Temperature dependence of gmin when T less than or equal to TPhase.}
#'       \item{\code{Q10_2_gmin [= 4.8]}: Temperature dependence of gmin when T greater than TPhase.}
...
#'       \item{\code{maximumRelativeGrowthRates [=   c(leaf = 0.05, cambium = 0.005, sapwood = 0.002, fineroot = 0.1)]}: Default maximum relative growth rates for leaves (m2 leaf ·cm-2 sapwood· day-1), tree sapwood (cm2 sapwood· cm-1 cambium · day-1), shrub sapwood (cm2 sapwood ·cm-2 sapwood· day-1) and fine roots (g dw · g dw -1 · day -1) when not specified via SpParams (\code{RGRleafmax}, \code{RGRcambiummax} , \code{RGRsapwoodmax} and \code{RGRfinerootmax}, respectively).}
...
#'       \item{\code{decompositionAnnualBaseRates}: Vector of base annual decomposition rates (in yr-1) for different pools.}
...
defaultControl<-function(transpirationMode = "Granier", 
  ...
  l <- list(
    ...
    leafConductanceEstimation = TRUE,
    rootConductivityEstimation = TRUE,
    ...
    maximumRelativeGrowthRates = list(leaf = 0.05, # m2 leaf ·cm-2 sapwood· day-1
    ...
    decompositionAnnualBaseRates = c("SurfaceMetabolic" = 8.0, 
                                     ...
                                     "CoarseRoots" = 0.1, 
                                     "SurfaceActive" = 6.0, 
                                     "BelowgroundActive" = 11.0, 
                                     "SurfaceSlow" = 0.08, 
                                     "BelowgroundSlow" = 0.4, 
                                     "BelowgroundPassive" = 0.0033),
