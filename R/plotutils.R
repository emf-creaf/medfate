.multiple_dynamics<-function(x, ylab=NULL, ylim = NULL) {
  df = data.frame(Y = as.vector(x), Date = as.Date(rownames(x)),
                  Cohort = gl(length(colnames(x)), nrow(x), labels=colnames(x)))
  g<-ggplot(df, aes(x=Date, y=Y))+
    geom_line(aes(col=Cohort, linetype = Cohort))+
    scale_color_discrete(name="")+
    scale_linetype_discrete(name="")
  if(!is.null(ylim)) g <- g+ylab(ylim)
  if(!is.null(ylab)) g <- g+ylab(ylab)
  return(g)
}
.single_dynamics<-function(x, ylab=NULL, ylim = NULL) {
