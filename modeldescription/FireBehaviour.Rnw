...
\begin{itemize}
\item{$P_{dead,i}$: Proportion of the plant that is dead.}
\item{$M_i$: Foliar moisture value (in percent of dry weight).}
\end{itemize}
Analogously, the same variables are needed for the herbaceous stratum.
\begin{itemize}
\item{$P_{dead,he}$: Proportion of herb fuels that respond to humidity changes as 1-h dead fuels.}
\item{$M_{live,he}$: Foliar moisture value of live herb fuels (in percent of dry weight).}
\end{itemize}

The model also needs the following input parameters:
\begin{itemize}
\item{$M_{dead}$: the moisture of 1-h dead fuels (in percent of dry weight).}
\item{$U$: Midflame windspeed (in $m\cdot s^{-1}$).}
\item{$S$: Slope (in percent).}
\end{itemize}


\section{Fuel characteristics}

\subsection{Cohort fuel loading}
...
\subsection{Vertical structure of fuels}
\subsubsection{Strata definition}
...
\begin{itemize}
\item{\textbf{Canopy}: Trees, snags and ladder fuels.}
\item{\textbf{Shrubs}: Primary and secondary layers.}
\item{\textbf{Non-woody vegetation} (herbs): grasses, sedges, rushes and forbs.}
\item{\textbf{Woody fuels}: All downed and dead wood, sound wood, rotten wood and stumps.}
\item{\textbf{Litter-lichen-moss}: Lichen, litter and moss layers.}
\item{\textbf{Ground fuels}: Duff, basal accumulation and squirrel middens.}
\end{itemize}
...
\subsubsection{Vertical distribution of tree and shrub fuels}
...
\begin{equation}\label{eq:CrownHeight}
H_{b,i} = H_{i} \cdot (1 - CR_i)
\end{equation}
In the case of shrubs the crown ratio is simply equal by a species-specific parameter: $CR_i =  cr(SP_i)$. In the case of trees, the crown ratio is modelled as a function of tree size and stand competition, following a modification of the logistic equation of Hasenauer \& Monserud (1996):
\begin{equation}
...
\subsubsection{Fuel bulk density profile}
Knowing at which height fuels are placed, the \textbf{fuel bulk density profile} (Reinhardt et al. 2006) is defined for any given interval $(H_1, H_2)$ as the bulk density ($kg/m^3$) of fine  fuels corresponding to that interval:
\begin{equation}
BDP(H_1, H_2) = \frac{\sum_{i} W_i(H_1, H_2)}{H_2-H_1}
\end{equation}
Canopy bulk density normally ranges between 0 and 0.4 $kg/m^3$ (Scott \& Reinhardt 2002). Sando and Wick (1972) arbitrarily defined canopy base height as the lower vertical 0.3-m section with a weight greater than $0.01124 kg/m^3$. A user-defined threshold $t_{BDP}$ (in $kg/m^3$) in 0.1-m sections is used to differentiate the surface fuelbed from canopy fuels. Using this threshold the model calculates the following three heights (Reinhardt et al. 2006):
\begin{itemize}
\item{\textbf{Shrub stratum base height}, $H_{sb}$ (in $cm$): the minimum height between 0 and 2 m where fuel bulk density is larger than $t_{BDP}$.} 
\item{\textbf{Shrub stratum top height}, $H_{st}$ (in $cm$): the maximum height between 0 and 2 m where fuel bulk density larger than $t_{BDP}$. With this definition $h_{s}$ cannot be higher than 2 m (corresponding to fuel model 4 in Anderson 1982).}
\item{\textbf{Canopy base height}, $H_{cb}$  (in $cm$): In terms of its consequences to crown fire initiation, canopy base height can be defined as the lowest height above the ground at which there is sufficient canopy fuel to propagate fire vertically through the canopy. It is calculated as the minimum height over $H_st$ when the bulk density starts again to be larger than $t_{BDP}$.}
\item{\textbf{Canopy top height}, $H_{ct}$ (in $cm$): the maximum height where bulk density is larger than $t_{BDP}$.}
\item{\textbf{Canopy gap}, $H_{gap}$ (in $cm$): the difference between $H_{cb}$ and $H_{st}$. The canopy gap is used to calculate crown initiation potential.}
\end{itemize}
The following figure illustrates the definition and analysis of the fuel bulk density profile for a given forest stand. Following Mitsopoulos \& Dimtrakopoulos (2007), a threshold $t_{BDP} = 0.04$ has been used to determine shrub and canopy heights.
\begin{center}
...
\subsection{Fuel loading ($w$) and fuel depth ($\delta$)}
\subsubsection{Canopy stratum}
...
\subsubsection{Non-woody stratum}
...
\subsubsection{Woody and litter strata}
...
Assuming a continuous input of litter, the variation in accumulated litter is described by a simple differential equation (Olson 1963; Birk and Simpson 1980):
\begin{equation}
...
\subsection{Other fuel characteristics}
...
\subsubsection{Particle density ($\rho_{p}$)}
...
\end{align}
\end{subequations}
where $\rho_p(SP_i)$ is the species-specific particle density. $\rho_{p, he}$, $\rho_{p, wo}$ and $\rho_{p, li}$ are all set to a default value $\rho_{p} = 400 kg\cdot m^{-3}= 25 lb\cdot ft^{-3}$ (Prichard et al. 2013). 

\subsubsection{Particle volume ($PV$)}
...
\end{equation}
\subsubsection{Packing ratio ($\beta$)}
...
\begin{subequations}\label{eq:packingratioshrub}
\begin{align}
...
\subsubsection{Surface-area-to-volume ratio ($\sigma$)}
...
\subsubsection{Fuel area index (FAI)}
...
\subsubsection{Proportion of dead fuel ($P_{dead}$)}
...
\subsubsection{Low heat content ($h$)}
...
\subsubsection{Flammability index ($\eta_{F}$)}
...
\subsubsection{Reactive volume ($RV$)}
...
\subsection{Unit conversion of fuel characteristics}
...
\begin{itemize}
\item{Loading: $1 kg\cdot m^{-2} = 0.204918 lb\cdot ft^{-2}$}
\item{Depths: $1m = 3.2808399ft$}
\item{Particle density and bulk density: $1 kg\cdot m^{-3} = 0.06242796 lb\cdot ft^{-3}$}
\item{Particle volume and reactive volume: $1 m^{3}\cdot m^{-2} = 3.2808399 ft^{3}\cdot ft^{-2}$}
\item{Surface-to-area-volume ratio: $1 m^{2}\cdot m^{-3} = 0.3048 ft^{2}\cdot ft^{-3}$}
\item{Heat content: $1kJ\cdot kg^{-1} = 0.429922614 Btu\cdot lb^{-1} $}
\item{Wind speed: $1 m \cdot s^{-1} = 2.23693629 mph$}
\end{itemize}
