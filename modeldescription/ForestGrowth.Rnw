...
Package \texttt{medfate} includes allows simulating plant growth of a set of cohorts (competing for light and water) in a single forest stand using function \texttt{growth()} or for multiple stands distributed spatially using function \texttt{growthpoints()}. Leaf area of plants can be severely decreased by drought stress, causing an increase in dead leaf area and affecting carbon and water fluxes. Functions \texttt{growth()} and \texttt{growthpoints()} may be used to:
\begin{itemize}
\item{Monitor or forecast temporal variation in water fluxes and soil water content in particular stands (for example to estimate regulation ecosystem services) taking into account processes determining plant growth.}
\item{Monitor or forecast temporal variation of plant drought stress in particular stands(for example to anticipate mortality events) taking into account processes determining plant growth.}
\item{Monitor or forecast temporal variation of live fuel moisture and the amount of standing dead and live fuels in particular stands (for example to monitor wildfire risk).}
\end{itemize}
...
Leaf area of each plant cohort is divided between live (wether in resistance buds or unfolded leaves) and dead (standing dead trees). Expanded leaf area corresponds to the portion of live leaf area that is unfolded at any given moment through the leaf phenological cycle. Leaf area density of individuals is considered constant across the crown. Sapwood area of individuals is another important state variable. The Pipe model (Shinozaki et al. 1964) is adopted to link increments of leaf area, sapwood area and fine root biomass. Ratios of leaf area to sapwood area (Huber value) can vary within species, due to environmental conditions (Mencuccini and Grace 1995). The model assumes a constant, species-specific Huber value, but allows deviations from the pipe model caused by drought-related leaf area reductions. 

Water fluxes, soil water balance and plant photosynthesis processes follow the design of the soil water balance model (function \texttt{swb()}; see De Cáceres et al. 2015 and vignette '\textbf{Soil water balance and drought stress}') and this part of the model design will not be repeated here. Plant respiration is calculated at the individual level, by estimating the respiration of leaves, stem and fine root compartments. While fine root respiration is proportional to leaf respiration, and hence to expanded leaf area, stem respiration depends on plant size.

Growth is determined taking into account environmental limitations onboth source (i.e. carbon assimilation) and sink (i.e. carbon investment on plant tissue expansion) (Fatichi et al. 2014; Guillemot et al. 2015; Körner 2015). With respect to carbon availability for growth, the model offers three alternatives. In the first one, carbon available for growth is simply the daily difference between net photosynthesis and maintenance respiration (i.e. no carbon storage). The second alternatives involves a single (fast) carbon storage pool that allows decoupling assimilation from growth. Finally, the third alternative involves two carbon storage pools ('fast' and 'slow') with a transfer rate between them (Richardson et al. 2013; Dietze et al. 2014), which we assume to be regulated by the need to maintain, as much as possible, a minimum amount of carbon in the fast pool (i.e. for metabolic and osmotic purposes). In the second and third modes, maximum overall C storage capacity is proportional to plant size.

The LPG model (Sitch et al. 2003) applies different turnover rates for different tissues, but then tries to satisfy the pipe model (Shinozaki et al. 1964) by allocating C where it is more limiting. Instead, we assume that baseline leaf and fine root turnover rates are linearly related to conversion from sapwood to heartwood. Similarly to 3-PG (Landsberg and Waring 1997) we assume that the turnover rate is smallest for young plants, and it increases up to a maximum value. The model assumes that plants cannot suffer from cavitation if the leaf water potential is large enough for growth to occur (i.e. if cell turgidity is large enough for cell elongation). Similarly, it also assumes that growth stops before cavitation starts during drought events. When sapwood area reduction occurs, this not only reduces leaf area, but also decreases the amount of fast C reserves available for future growth. Thus it is assumed that parts of the plant are effectively disconnected.

Tree structural variables are updated as in forest gap models (e.g., Lindner et al. 1997)
...
print(s)
@
Soil input for function \texttt{growth()} is an object of class \texttt{soil} that is created using a function with the same name. Function \texttt{soil()} calculates water content at field capacity and initializes soil moisture to a given level:
<<>>=
examplesoil = soil(s, c(1.0, 1.0, 1.0))
print(examplesoil)
@
...
data("exampleforest")
fo <- emptyforest()
fo$treeData<-exampleforest$treeData[1,]
fo$treeData$Z50 = 300
fo$treeData$Z95 = 2000
fo$treeData$N = 500
fo$shrubData<-exampleforest$shrubData[1,]
fo$shrubData$Cover = 50
fo$shrubData$Height = 70
fo$shrubData$Z = 500
control <- defaultControl()
control$verbose = FALSE
control$storagePool = "two"
x <-forest2growthInput(fo, examplesoil, SpParamsMED, control)
@
The following is the result of calling function \texttt{forest2growthInput} for a forest stand with a single cohort with 500 trees/ha where all individuals are 8 m height and 37.55 cm DBH:
<<>>=
x
@
...
\subsection{Water balance and photosynthesis}

\begin{center}
<<fig=TRUE, echo=FALSE, width=5, height=4>>=
par(mar=c(4,4,1,4))
plot(dates,rowSums(S1$PlantPhotosynthesis), type="l", col="green", xlab="",ylab="Net photosynthesis (gC/m2)")
lines(dates,rowSums(S1$PlantTranspiration)*4, type="l", col="blue")
axis(4, at=c(0,4,8,12), labels=c(0,1,2,3))
mtext("Transpiration (mm)", 4, line=2)
legend("topleft", lty=c(1,1), legend = c("Net photosynthesis","Transpiration"), col=c("green", "blue"), bty="n")
@
\end{center}
...
plot(dates,S1$PlantPhotosynthesis[,1], type="l", xlab="",ylab="Photosynthesis per ground area (gC/m2)")
lines(dates, S1$PlantPhotosynthesis[,2], lty=2)

x1 = (S1$PlantPhotosynthesis[,1])/(x$above$N[1]/10000)#/S1$PlantLAIlive[,2]
x2 = (S1$PlantPhotosynthesis[,2])/(x$above$N[2]/10000)#/S1$PlantLAIlive[,2]
plot(dates,x1, type="l", xlab="",ylab="Photosynthesis per individual (gC/ind)")
lines(dates, x2, lty=2)
abline(h=0, col="gray")

plot(dates,S1$PlantTranspiration[,1], type="l", xlab="",ylab="Transpiration per ground area (mm/m2)")
lines(dates, S1$PlantTranspiration[,2], lty=2)
abline(h=0, col="gray")

x1 = (S1$PlantTranspiration[,1])/(x$above$N[1]/10000)
x2 = (S1$PlantTranspiration[,2])/(x$above$N[2]/10000)
plot(dates,x1, type="l", xlab="",ylab="Transpiration per individual (mm/ind)")
lines(dates, x2, lty=2)
abline(h=0, col="gray")
@
\end{center}
...
plot(S1, "Psi", xlab="")
plot(dates,S1$PlantPsi[,1], type="l", ylab="Leaf water potential (MPa)", xlab="",ylim=c(-5,0))
lines(dates,S1$PlantPsi[,2], lty=2)
legend("bottomleft", lty=c(1,2), legend = c("Tree individuals","Shrub individuals"), bty="n")
@
\end{center}

\subsection{Respiration and carbon balance}

\begin{center}
<<fig=TRUE, echo=FALSE, width=8, height=8>>=
par(mar=c(4,4,1,4), mfrow=c(2,2))
plot(dates,S1$PlantRespiration[,1], type="l", xlab="",ylab="Respiration per ground area (gC/m2)", ylim=c(0,2))
lines(dates, S1$PlantRespiration[,2], lty=2)

x1 = (S1$PlantRespiration[,1])/(x$above$N[1]/10000)#/S1$PlantLAIlive[,2]
x2 = (S1$PlantRespiration[,2])/(x$above$N[2]/10000)#/S1$PlantLAIlive[,2]
plot(dates,x1, type="l", xlab="",ylab="Respiration per individual (gC/ind)", ylim=c(0,35))
lines(dates, x2, lty=2)
abline(h=0, col="gray")

plot(dates,S1$PlantPhotosynthesis[,1]-S1$PlantRespiration[,1], type="l", xlab="",ylab="Photosynthesis-Respiration per ground area (gC/m2)", ylim=c(-1,7))
lines(dates, S1$PlantPhotosynthesis[,2]-S1$PlantRespiration[,2], lty=2)
abline(h=0, col="gray")

x1 = (S1$PlantPhotosynthesis[,1]-S1$PlantRespiration[,1])/(x$above$N[1]/10000)
x2 = (S1$PlantPhotosynthesis[,2]-S1$PlantRespiration[,2])/(x$above$N[2]/10000)
...
<<fig=TRUE, echo=FALSE, width=5, height=4>>=
par(mar=c(4,4,1,4))
plot(dates,100*S1$PlantSAgrowth[,1]/S1$PlantSA[,1], type="l", xlab="",ylab="Relative growth rate (cm2·cm-2)")
lines(dates,100*S1$PlantSAgrowth[,2]/S1$PlantSA[,2],lty=2)
# axis(4, at=c(0,4,8,12), labels=c(0,1,2,3))
# mtext("Transpiration (mm)", 4, line=2)
...
<<fig=TRUE, echo=FALSE, width=5, height=4>>=
par(mar=c(4,4,1,4))
plot(dates,S1$PlantLAIlive[,1], type="l", xlab="",ylab="Live leaf area index (m2·m-2)", ylim=c(0,2.5))
lines(dates,S1$PlantLAIlive[,2],lty=2)
# axis(4, at=c(0,4,8,12), labels=c(0,1,2,3))
# mtext("Transpiration (mm)", 4, line=2)
legend("topleft", lty=c(1,2), legend = c("Tree individuals","Shrub individuals"), bty="n")
@
...
\subsection{Dynamics of carbon pools}

\begin{center}
<<fig=TRUE, echo=FALSE, width=5, height=4>>=
par(mar=c(4,4,1,4))
plot(dates,100*S1$PlantCstorageFast[,1], type="l", ylab="Fast C pool (%)", xlab="", ylim=c(0,100))
lines(dates,100*S1$PlantCstorageFast[,2], lty=2)
legend("bottomleft", lty=c(1,2), legend = c("Tree individuals","Shrub individuals"), bty="n")
@
\end{center}
...
<<fig=TRUE, echo=FALSE, width=5, height=4>>=
par(mar=c(4,4,1,4))
plot(dates,100*S1$PlantCstorageSlow[,1], type="l", ylab="Slow C pool (%)", xlab="", ylim=c(0,100))
lines(dates,100*S1$PlantCstorageSlow[,2], lty=2)
legend("bottomleft", lty=c(1,2), legend = c("Tree individuals","Shrub individuals"), bty="n")
@
