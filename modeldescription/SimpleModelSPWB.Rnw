...
\usepackage{hyperref}
\usepackage{amsmath}
\title{Simple model: Water balance and plant drought stress}
...
model, under different situations (see function \texttt{spwb.RainInterception}). 
...
The amount of water infiltrating into the soil is $P_{net} + Sm - Ru$, where $Sm$ is snow melt (only if snow pack is considered) and $Ru$ is the water lost by runoff (see function \texttt{spwb.SoilInfiltration}). Following
...
\texttt{spwb.SoilInfiltration}), $V_{soil}$, calculated from different soil depths
...
RunEvaporation<-function(Gsoil, Ksoil, d1,d2,d3, numDays = 15){
  ...
  for(i in 1:numDays){
    ...
    Esoil[i] = spwb.SoilEvaporation(DEF=(MaxVol1*(1 - W1[i])), PETs = PET*Lground, Gsoil = Gsoil)
