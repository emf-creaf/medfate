...
model, under different situations (see function \texttt{hydrology.rainInterception}). 
...
\texttt{hydrology.soilInfiltration}), $V_{soil}$, calculated from different soil depths
...
\texttt{hydrology.soilEvaporation}): 
\begin{equation}
...
RunEvaporation<-function(Gsoil, Ksoil, d1,d2,d3, numDays = 15){
  ...
  for(i in 1:numDays){
    ...
    Esoil[i] = hydrology.soilEvaporation(DEF=(MaxVol1*(1 - W1[i])), PETs = PET*Lground, Gsoil = Gsoil)
