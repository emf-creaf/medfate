...
\subsubsection{Plant water storage}

Calculation of Vmax [m3/m2] from H (m) and leaf area to sapwood area ratio (m2/m2):
\begin{equation}
V_{sapwood} = H \cdot H_v \cdot \Theta_{sapwood}
\end{equation}
where $H_v$ is the Huber value (m$^2$ of sapwood area per m$^2$ of leaf area) and $\Theta_{sapwood} is sapwood porosity (cm$^3$ of water per cm$^3$ of tissue), which can be derived from wood density ($W_{dens}):
\begin{equation}
\Theta_{sapwood} = 1 - (W_{dens} / 1.54)
\end{equation}
density of wood substance can be fixed to 1.54 g·cm$^{-3}$ (Dunlap 1914). For example, wood densities ranging from 0.443 to 1.000 g·cm$^{-3}$ result in sapwood porosity values between 0.35 and 0.71.
