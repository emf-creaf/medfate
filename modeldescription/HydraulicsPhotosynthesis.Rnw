...
\subsubsection{Pressure-volume curves of the symplastic fraction}
A pressure-volume curve of a tissue relates a water potential (normally $\Psi_{W}$; MPa) against relative water content ($RWC$; kgH$_2$O kg$^{-1}$H$_2$O at saturation) in drying tissues. Pressure-volume theory is usually applied to leaves (Bartlett et al. 2012), but it can also be applied to sapwood. For living cells, the relationship between $\Psi$ and $RWC$ of the symplasmic fraction ($RWC_{sym}$) is achieved by separating $\Psi_{W}$ into osmotic (solute) potential ($\Psi_{S}$) and the turgor potential ($\Psi_{P}$):
\begin{equation}
...
In the case of leaves, their overall relative water content can be obtained from $\Psi_{leaf}$ by calculating  $RWC_{sym}(\Psi_{leaf})$ followed by assuming a constant apoplastic fraction $a_{f,l}$:
\begin{equation}
RWC(\Psi_{leaf}) = a_{f,l} + RWC_{sym}(\Psi_{leaf}) \cdot (1 - a_{f,l})
\end{equation}
Following Martin-StPaul et al. (2017), we consider two different plant water reservoirs in the stem (Tyree \& Yang 1990). The first is the apoplastic reservoir, consisting of inelastic xylem cells that release their water to the transpiration stream following embolism. As in Hölttä et al. (2009), we equate the relative water content of the apoplastic reservoir of the xylem to the proportion of maximum conductance in the vulnerability curve:
\begin{equation}
RWC_{apo}(\Psi_{stem}) = \frac{k_x(\Psi_{stem})}{k_{x,max}} = e^{-((\Psi/d_{stem})^c_{stem})}
\end{equation}
The second reservoir is formed by more elastic living cells in the stem (cambium, phloem and bark cells) and can potentially be a larger source of water during relatively high water potentials. This source can be described using the relative water content of the symplasm, as done for leaves. By knowing the proportion of apoplastic tissue in the stem, $a_{f,s}$, we can obtain the overall relative water content in the stem as:
\begin{equation}
RWC(\Psi_{stem}) = RWC_{apo}(\Psi_{stem}) \cdot a_{f,s} + RWC_{sym}(\Psi_{stem}) \cdot (1 - a_{f,s}) 
\end{equation}

Once we know $RWC(\Psi_{stem})$ we can calculate the amount of water per unit of water storage tissue volume in the stem ($W_{tissue}$ in kgH$_2$O$\cdot$m$^{-3}$):
\begin{equation}
W_{tissue}(\Psi_{stem}) = RWC(\Psi_{stem}) \cdot \rho_{w} \cdot \Theta_{sat}
\end{equation}
where $\rho_{w}$ is the density of water (kgH$_2$O$\cdot$m$^{-3}$) and $\Theta_{sat}$ (m$^3$H$_2$O$\cdot$m$^{-3}$) is tissue porosity, i.e. the maximum volumetric water content per volume of stem tissue. If, in addition, we know $V_{storage}$ the volume of storage tissue per leaf area, then the amount of water that is stored per leaf area will be: 
\begin{equation}
W_{leaf}(\Psi_{stem}) = V_{storage} \cdot W_{tissue}(\Psi_{stem})
\end{equation}
...
\subsection{Stem storage compartments}
Storage compartments contribute to sapflow dynamics by exchanging water with xylem. We will consider here a single symplastic storage compartment connected to the stem sapwood, assuming that this is the most important water storage compartment in the plant. Additionally, we track of changes in water volume within sapwood tissue itself. Considering this apoplastic compartment allows calculating the release of water when cavitation occurs and, conversely, simulate the process of refilling from the symplastic storage compartment.

\subsubsection{Effect of storage compartments on flow}
So far, we have been concerned with steady-state flows of a hydraulic network assuming no water storage compartments. Water potential at the upper end of the stem ($\Psi_{stem}$) was obtained integrating the stem xylem vulnerability curve, using $\Psi_{up,k}=\Psi_{rootcrown}$ and making the result equal to the input flow from the root system. If we consider the two storage compartments, the downstream instantaneous flow may not necessarily be equal to the flow entering the stem:
\begin{equation}
E_{stem} = E_{root} - (\Delta{S_{apo}}/\Delta{t}) - (\Delta{S_{sym}}/\Delta{t})
\end{equation}
where $E_{stem}$ is the stem outflow, $E_{root}$ is the input flow (from the root system) and $\Delta{S_{apo}}/\Delta{t}$ and  $\Delta{S_{sym}}/\Delta{t}$ is the change in water of the symplastic and apoplastic storage compartments. If $\Delta{S_{sym}}/\Delta{t}$ is negative, then there is a flow from the compartment to the xylem. Otherwise, flow is from the xylem to the storage compartment. $\Delta{S_{apo}}/\Delta{t}$ is a term to increase or decrease flow as a result of changes in stem water potential.

Here we assume that the storage compartment has a common water potential $\Psi_{storage}$ and the flow between the compartment at any height of the stem depends on the difference in water potential between the compartment and the xylem:

\begin{equation}
\Delta{S_{sym}}/\Delta{t}7 = \int_{\Psi_{stem}}^{\Psi_{rootcrown}}{f_{storage}(\Psi) d\Psi}
\end{equation}
