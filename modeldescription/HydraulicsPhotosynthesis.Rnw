...
\title{Complex model: Hydraulics, stomatal conductance and photosynthesis}
...
The supply-loss theory of plant hydraulics, recently presented by Sperry and Love (2015) and tested in Sperry et al. (2016a), uses the physics of flow through soil and xylem to quantify how canopy water supply declines with drought and ceases by hydraulic failure. The theory builds on a previous hydraulic model (Sperry et al. 1998) and can be applied to different ways of representing the soil-plant continuum. In our case we considered a network of $(N \times 2 + 2)$ resistance elements, with soil being represented in $N$ different layers. For each soil layer there is a rhizosphere element in series with a root xylem element. The $N$ soil layers are in parallel up to the root crown. From there there are two final elements: the stem xylem and the leaf.
...
This document includes the design decisions taken to implement Sperry and Love (2015) supply functions and Sperry et al (2016a) stomatal regulation model in \texttt{medfate}, including the need to separate sunlit and shade leaves to scale photosynthesis and cost-benefit maximization to the plant level. Althougth the model implements a network representation of the soil-plant continuum, simpler one-element, two-element and three-element representations will be used in this document to facilitate understanding. We include a section devoted to explain how hydraulic parameters of the continuum are estimated from model inputs (i.e. tissue hydraulic properties). Finally, we include examples to illustrate the theory under different situations comprising different soil types, soil water potentials and with/without cavitated conduits.
...
Xylem tissues (roots and stem) are assigned a two-parameter Weibull function as the vulnerability curve $k_x(\Psi)$:
\begin{equation}\label{eq:xylemvulnerability}
k_x(\Psi) = k_{x,max}\cdot e^{-((\Psi/d)^c)}
\end{equation}
where $k_{x,max}$ is the xylem maximum hydraulic conductance (defined as flow per surface unit and per pressure drop), and $c$ and $d$ are species-specific and tissue-specific parameters. Note that parameter $d$ is the water potential (in MPa) at which $k_{x}(\Psi)/k_{x,max} = e^{-1} = 0.367$. Parameter $c$ controls the shape of the vulnerability curve (‘exponential’ shape with no threshold has $c \leq 1$, sigmoidal threshold has $c > 1$). For example, we define the following parameter values for a stem xylem ($k_{s,max}$ and parameters of the vulnerability curve; see \texttt{VCstem\_c} and \texttt{VCstem\_d} in \texttt{SpParamsMED}):
<<>>==
kstemmax = 5.0 # in mmol·m-2·s-1·MPa-1
stemc = 3 
stemd = -3.0 # in MPa
@
For root xylem ($k_{r,max}$), we may assume a higher conductance (i.e. higher efficiency) but also higher vulnerability to cavitation (see \texttt{VCroot\_c} and \texttt{VCroot\_d} in \texttt{SpParamsMED}):
<<>>==
krootmax = 6.6
rootc = 2
rootd = -2.5 #MPa
...
k_l(\Psi) = k_{l,max}\cdot e^{-((\Psi/d)^{c})}
\end{equation}
where $k_{l,max}$ is the leaf maximum hydraulic conductance (\texttt{VCleaf\_kmax} in \texttt{SpParamsMED}). Values defined below specify higher conductance for leaves but also slightly higher vulnerability (see parameters \texttt{VCleaf\_c} and \texttt{VCleaf\_d} in \texttt{SpParamsMED}):
<<>>==
kleafmax = 10
leafc = 2
leafd = -2
...
With these parameter values, the vulnerability curves for root, stem and leaf are (see \texttt{hydraulics.xylemConductance()}):
\begin{center}
<<fig=TRUE, width=5, height=5, echo=FALSE>>==
psiCav = -2.5
par(mar=c(4,4,1,1), mfrow=c(1,1))
kleaf = unlist(lapply(psiVec, hydraulics.xylemConductance, kleafmax, leafc, leafd))
kstem = unlist(lapply(psiVec, hydraulics.xylemConductance, kstemmax, stemc, stemd))
kroot = unlist(lapply(psiVec, hydraulics.xylemConductance, krootmax, rootc, rootd))
...
v &=& [(\alpha \Psi)^{n}+1]^{-1}
\end{eqnarray}
where $k_{rh,max}$ is the maximum rhizosphere conductance, and $n$ and $\alpha$ are texture-specific parameters (see Leij et al. 1996; Carsel \& Parrish 1988). These are automatically set by function \texttt{soil()} when initializing soil objects (see parameters \texttt{VG\_alpha} and \texttt{VG\_n} in the output of \texttt{soil()}), but we can use function \texttt{soil.vanGenuchtenParamsCarsel()} to derive them from texture types:
<<>>=
textures = c("Sandy loam","Silt loam", "Clay")
#Textural parameters
#Sandy clay loam 
p1 = soil.vanGenuchtenParamsCarsel(textures[1])
p1
alpha1 = p1[1]  
n1 = p1[2]
#Silt loam
p2 = soil.vanGenuchtenParamsCarsel(textures[2])
p2
alpha2 =  p2[1]
n2 = p2[2]
#Silty clay
p3 = soil.vanGenuchtenParamsCarsel(textures[3])
p3
alpha3 =  p3[1]
n3 = p3[2]
@
We can estimate maximum rhizosphere conductance values assuming that they account for an average percentage of the resistance (e.g. 15\%) across the continuum (see functions \texttt{hydraulics.averageRhizosphereResistancePercent()} and \texttt{hydraulics.findRhizosphereMaximumConductance()}):
<<>>=
percentResistance = 15
#Sandy clay loam 
krmax1 = hydraulics.findRhizosphereMaximumConductance(percentResistance, 
                      n1,alpha1, krootmax, rootc,rootd, kstemmax, stemc, stemd,
                      kleafmax, leafc, leafd)
krmax1
#Silt loam
krmax2 = hydraulics.findRhizosphereMaximumConductance(percentResistance, 
                      n2,alpha2, krootmax, rootc,rootd, kstemmax, stemc, stemd,
                      kleafmax, leafc, leafd)
krmax2
#Silty clay
krmax3 = hydraulics.findRhizosphereMaximumConductance(percentResistance, 
                      n3,alpha3, krootmax, rootc,rootd, kstemmax, stemc, stemd,
                      kleafmax, leafc, leafd)
krmax3
@
With these parameters, the resulting $k_{rh}(\Psi)$ functions can be displayed using the function \texttt{hydraulics.vanGenuchtenConductance()}:
\begin{center}
...
A pressure-volume curve of a tissue relates a water potential (normally $\Psi_{W}$; MPa) against relative water content ($RWC$; kgH$_2$O·kg$^{-1}$H$_2$O at saturation) in drying tissues. Pressure-volume theory is usually applied to leaves (Bartlett et al. 2012), but it can also be applied to other tissues such as sapwood or cambium cells. 

For living cells, the relationship between $\Psi$ and $RWC$ of the symplasmic fraction ($RWC_{sym}$) is achieved by separating $\Psi_{W}$ into osmotic (solute) potential ($\Psi_{S}$) and the turgor potential ($\Psi_{P}$):
\begin{equation}
...
\end{equation}
When $\Psi_{W} \leq \Psi_{tlp}$, the water potential at turgor loss point, then $\Psi_{P} = 0$ and $\Psi_{W} = \Psi_{S}$. If $\Psi_{W} > \Psi_{tlp}$ then the two components are needed. The water potential at turgor loss point ($\Psi_{tlp}$) can be found by (Bartlett et al. 2012):
\begin{equation}
...
Apoplastic reservoirs (e.g. sapwood) consist of inelastic cells that release their water to the transpiration stream following embolism. As in Hölttä et al. (2009), we equate the relative water content of the apoplastic reservoir of the xylem to the proportion of maximum conductance in the vulnerability curve:
\begin{equation}
RWC_{apo}(\Psi_{stem}) = \frac{k_x(\Psi_{stem})}{k_{x,max}} = e^{-((\Psi/d_{stem})^{c_{stem}})}
\end{equation}

<<fig=TRUE, width=4, height=4, echo=FALSE>>==
psi = seq(-7,0, by=0.1)
rwc_a = rep(NA, length(psi))
for(i in 1:length(psi)) rwc_a[i] = moisture.apoplasticRWC(psi[i],stemc,stemd)
plot(psi, rwc_a, type="l", xlab="Water potential (MPa)", ylab = "Apoplastic RWC")
@


\subsubsection{Water content in leaves and stem}
The overall relative water content of leaves can be obtained from $\Psi_{leaf}$ by calculating  $RWC_{sym}(\Psi_{leaf})$ and $RWC_{apo}(\Psi_{leaf})$ followed by assuming a constant apoplastic fraction $f_{apo,l}$:
\begin{equation}
RWC(\Psi_{leaf}) = RWC_{apo}(\Psi_{leaf}) \cdot f_{apo,l} + RWC_{sym}(\Psi_{leaf}) \cdot (1 - f_{apo,l})
\end{equation}
$RWC(\Psi_{leaf})$ is used in \texttt{medfate} to calculate live fuel moisture content, but relative water content of leaves does not intervene in dynamic hydraulic calculations.

Following Martin-StPaul et al. (2017), we consider two sources of water in the stem xylem (Tyree \& Yang 1990). The first comes from the conduits (tracheids or vessels), which will release water due to cavitation and may be refilled with water from adjacent living tissue (from the second source). If we know $RWC_{apo}(\Psi_{stem})$, the relative water content in the xylem conduits, and $V_{sapwood}$ (in m$^3$), the volume of functional sapwood in the plant, then the amount of water that is stored in conduits is: 
\begin{equation}
S_{apo}(\Psi_{stem}) = V_{sapwood} \cdot f_{apo, s} \cdot RWC_{apo}(\Psi_{stem}) \cdot \rho_{w}
\end{equation}
where $\rho_{w}$ is the density of water (kgH$_2$O$\cdot$m$^{-3}$) and $f_{apo,s}$ is the volume fraction of apoplastic tissue within sapwood. The second source of water is formed by more elastic living cells in the stem (i.e. parenchyma or rays) and can potentially be a large source of water during relatively high water potentials. This source can be described using the relative water content of a symplasmic tissue. This xylem storage compartment has its own water potential and exchanges water with the xylem conduits according to the difference in water potential. The amount of water stored in the symplastic compartment at any time is:
\begin{equation}
S_{sym}(\Psi_{sym}) = V_{sapwood} \cdot (1 - f_{apo, s}) \cdot RWC_{sym}(\Psi_{storage}) \cdot \rho_{w}
\end{equation}
...
psiSoil3 = -2.0
psiSoil4 = -3.0
E1 = unlist(lapply(psiVec, hydraulics.EXylem, psiSoil1, kstemmax, stemc, stemd))
E2 = unlist(lapply(psiVec, hydraulics.EXylem, psiSoil2, kstemmax, stemc, stemd))
E3 = unlist(lapply(psiVec, hydraulics.EXylem, psiSoil3, kstemmax, stemc, stemd))
E4 = unlist(lapply(psiVec, hydraulics.EXylem, psiSoil4, kstemmax, stemc, stemd))
E1emb = unlist(lapply(psiVec, hydraulics.EXylem, psiSoil1, kstemmax, stemc, stemd, psiCav = psiCav))
E2emb = unlist(lapply(psiVec, hydraulics.EXylem, psiSoil2, kstemmax, stemc, stemd, psiCav = psiCav))
E3emb = unlist(lapply(psiVec, hydraulics.EXylem, psiSoil3, kstemmax, stemc, stemd, psiCav = psiCav))
E4emb = unlist(lapply(psiVec, hydraulics.EXylem, psiSoil4, kstemmax, stemc, stemd, psiCav = psiCav))
E1[E1<0] = NA
E2[E2<0] = NA
E3[E3<0] = NA
E4[E4<0] = NA
...
E3emb[E3emb<0] = NA
E4emb[E4emb<0] = NA
Psic = hydraulics.psiCrit(stemc,stemd)
...
The supply function of the rhizosphere element relates the flow rate to the pressure inside the roots ($\Psi_{root}$). It is calculated by numerical integration of the van Genuchten function (see function \texttt{hydraulics.E2psiVanGenuchten()}). Here we draw the supply function for the rhizosphere starting at the four different values of bulk soil pressure ($\Psi_{soil}$) and for the same three texture types:
\begin{center}
<<fig=TRUE, width=4, height=4, echo=FALSE>>==
par(mar=c(4,4,1,1))
Evec = seq(0, 2, by=0.01)
...
plot(-psiVG11, Evec, type="l", ylab="Rhizosphere flow rate (mmol·m-2·s-1)", xlab = "Root pressure (-MPa)",  xlim=c(0, 7), col=col1)
lines(-psiVG21, Evec, lty=2, lwd=1.5, col=col1)
lines(-psiVG31, Evec, lty=3, lwd=1.5, col=col1)
lines(-psiVG12, Evec, lty=1, lwd=1.5, col=col2)
lines(-psiVG22, Evec, lty=2, lwd=1.5, col=col2)
lines(-psiVG32, Evec, lty=3, lwd=1.5, col=col2)
lines(-psiVG13, Evec, lty=1, lwd=1.5, col=col3)
lines(-psiVG23, Evec, lty=2, lwd=1.5, col=col3)
lines(-psiVG33, Evec, lty=3, lwd=1.5, col=col3)
lines(-psiVG14, Evec, lty=1, lwd=1.5, col=col4)
lines(-psiVG24, Evec, lty=2, lwd=1.5, col=col4)
lines(-psiVG34, Evec, lty=3, lwd=1.5, col=col4)
...
Let us describe the soil-plant continuum is represented using \emph{two} elements in series (rhizosphere + stem xylem). In this case, the supply function has to be calculated by sequentially using the previous supply functions. The $E_i$ is identical for each element and equal to the canopy $E$. Since $\Psi_{soil}$ is known, one first inverts the supply function of the rhizosphere to find $\Psi_{root}$ (see function \texttt{hydraulics.E2psiVanGenuchten()}) and then inverts the supply function of the xylem to find $\Psi_{canopy}$ (see function \texttt{hydraulics.E2psiXylem()}). The two operations can be summarized in a single supply function describing the potential rate of water supply for transpiration ($E$) as function of the canopy xylem pressure ($\Psi_{canopy}$), starting from different bulk soil ($\Psi_{soil}$) values (see function \texttt{hydraulics.supplyFunctionTwoElements()}): 

\begin{center}
<<fig=TRUE, width=8, height=4.5, echo=FALSE>>==
par(mar=c(4,4,2,1), mfrow=c(1,2))
psi2E11 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil1, krmax1, kstemmax, n1, alpha1, stemc,stemd, 0.0, dE)
psi2E12 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil2, krmax1, kstemmax, n1, alpha1, stemc,stemd, 0.0, dE)
psi2E13 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil3, krmax1, kstemmax, n1, alpha1, stemc,stemd, 0.0, dE)
psi2E14 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil4, krmax1, kstemmax, n1, alpha1, stemc,stemd, 0.0, dE)
psi2E21 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil1, krmax2, kstemmax, n2, alpha2, stemc,stemd, 0.0, dE)
psi2E22 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil2, krmax2, kstemmax, n2, alpha2, stemc,stemd, 0.0, dE)
psi2E23 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil3, krmax2, kstemmax, n2, alpha2, stemc,stemd, 0.0, dE)
psi2E24 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil4, krmax2, kstemmax, n2, alpha2, stemc,stemd, 0.0, dE)
psi2E31 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil1, krmax3, kstemmax, n3, alpha3, stemc,stemd, 0.0, dE)
psi2E32 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil2, krmax3, kstemmax, n3, alpha3, stemc,stemd, 0.0, dE)
psi2E33 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil3, krmax3, kstemmax, n3, alpha3, stemc,stemd, 0.0, dE)
psi2E34 = hydraulics.supplyFunctionTwoElements(Emax, psiSoil4, krmax3, kstemmax, n3, alpha3, stemc,stemd, 0.0, dE)
...
psi2E11emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil1, krmax1, kstemmax, n1, alpha1, stemc,stemd,psiCav, dE)
psi2E12emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil2, krmax1, kstemmax, n1, alpha1, stemc,stemd, psiCav, dE)
psi2E13emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil3, krmax1, kstemmax, n1, alpha1, stemc,stemd, psiCav, dE)
psi2E14emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil4, krmax1, kstemmax, n1, alpha1, stemc,stemd, psiCav, dE)
psi2E21emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil1, krmax2, kstemmax, n2, alpha2, stemc,stemd, psiCav, dE)
psi2E22emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil2, krmax2, kstemmax, n2, alpha2, stemc,stemd, psiCav, dE)
psi2E23emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil3, krmax2, kstemmax, n2, alpha2, stemc,stemd, psiCav, dE)
psi2E24emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil4, krmax2, kstemmax, n2, alpha2, stemc,stemd, psiCav, dE)
psi2E31emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil1, krmax3, kstemmax, n3, alpha3, stemc,stemd, psiCav, dE)
psi2E32emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil2, krmax3, kstemmax, n3, alpha3, stemc,stemd, psiCav, dE)
psi2E33emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil3, krmax3, kstemmax, n3, alpha3, stemc,stemd, psiCav, dE)
psi2E34emb = hydraulics.supplyFunctionTwoElements(Emax, psiSoil4, krmax3, kstemmax, n3, alpha3, stemc,stemd, psiCav, dE)
...
1/sum(rvec)
@
As before, the supply function has to be calculated by sequentially. The $E_i$ is identical for each element. Since $\Psi_{soil}$ is known, one first inverts the supply function of the rhizosphere to find $\Psi_{root}$ and then inverts the supply function of the xylem to find $\Psi_{stem}$. Finally, one inverts the supply function of the leaf element to find $\Psi_{leaf}$. As before, the three operations can be summarized in a single supply function describing the potential rate of water supply for transpiration ($E$) as function of the leaf pressure ($\Psi_{leaf}$), starting from different bulk soil ($\Psi_{soil}$) values (see function \texttt{hydraulics.supplyFunctionThreeElements()}):

\begin{center}
<<fig=TRUE, width=8, height=4.5, echo=FALSE>>==
par(mar=c(4,4,2,1), mfrow=c(1,2))
psi2E11 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil1, krmax1, kstemmax, kleafmax, n1, alpha1, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E12 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil2, krmax1, kstemmax, kleafmax, n1, alpha1, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E13 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil3, krmax1, kstemmax, kleafmax, n1, alpha1, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E14 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil4, krmax1, kstemmax, kleafmax, n1, alpha1, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E21 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil1, krmax2, kstemmax, kleafmax,n2, alpha2, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E22 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil2, krmax2, kstemmax, kleafmax,n2, alpha2, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E23 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil3, krmax2, kstemmax, kleafmax,n2, alpha2, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E24 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil4, krmax2, kstemmax, kleafmax,n2, alpha2, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E31 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil1, krmax3, kstemmax, kleafmax,n3, alpha3, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E32 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil2, krmax3, kstemmax, kleafmax,n3, alpha3, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E33 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil3, krmax3, kstemmax, kleafmax,n3, alpha3, stemc,stemd, leafc,leafd, 0.0, dE)
psi2E34 = hydraulics.supplyFunctionThreeElements(Emax, psiSoil4, krmax3, kstemmax, kleafmax,n3, alpha3, stemc,stemd, leafc,leafd, 0.0, dE)
mm = max(psi2E11$FittedE)
...
psi2E11emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil1, krmax1, kstemmax, kleafmax,n1, alpha1, stemc,stemd, leafc,leafd,psiCav, dE)
psi2E12emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil2, krmax1, kstemmax, kleafmax,n1, alpha1, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E13emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil3, krmax1, kstemmax, kleafmax,n1, alpha1, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E14emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil4, krmax1, kstemmax, kleafmax,n1, alpha1, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E21emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil1, krmax2, kstemmax, kleafmax,n2, alpha2, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E22emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil2, krmax2, kstemmax, kleafmax,n2, alpha2, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E23emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil3, krmax2, kstemmax, kleafmax,n2, alpha2, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E24emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil4, krmax2, kstemmax, kleafmax,n2, alpha2, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E31emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil1, krmax3, kstemmax, kleafmax,n3, alpha3, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E32emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil2, krmax3, kstemmax, kleafmax,n3, alpha3, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E33emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil3, krmax3, kstemmax, kleafmax,n3, alpha3, stemc,stemd, leafc,leafd, psiCav, dE)
psi2E34emb = hydraulics.supplyFunctionThreeElements(Emax, psiSoil4, krmax3, kstemmax, kleafmax,n3, alpha3, stemc,stemd, leafc,leafd, psiCav, dE)
...
\end{eqnarray}
where $E_{k, rhizosphere}$ and $E_{k, root}$ are calculated using either van Genuchten or Weibull function as vulnerability curves, respectively. In the case of rhizosphere elements, $\Psi_{up,k}=\Psi_{soil,k}$ and in the case of root elements $\Psi_{up,k}=\Psi_{rootsurf,k}$. Solving the steady-state equations also provides values for flow across each of the parallel paths $E_{k, rhizosphere} = E_{k, root}$, which are useful to conduct water balance operations on each layer. 
...
v = root.ldrDistribution(Z50, Z95, d)
v
@
...
Having all these maximum conductances, we can now build the supply functions for each soil texture and starting from the different soil water potential configurations (see function \texttt{hydraulics.supplyFunctionRootSystem()}): 
\begin{center}
<<fig=TRUE, width=8, height=4.5, echo=FALSE>>==
par(mar=c(4,4,2,1), mfrow=c(1,2))
psiMax = -7.0
supplyRS11 = hydraulics.supplyFunctionRootSystem(psiSoilLayers1,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS12 = hydraulics.supplyFunctionRootSystem(psiSoilLayers2,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS13 = hydraulics.supplyFunctionRootSystem(psiSoilLayers3,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS14 = hydraulics.supplyFunctionRootSystem(psiSoilLayers4,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS21 = hydraulics.supplyFunctionRootSystem(psiSoilLayers1,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS22 = hydraulics.supplyFunctionRootSystem(psiSoilLayers2,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS23 = hydraulics.supplyFunctionRootSystem(psiSoilLayers3,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS24 = hydraulics.supplyFunctionRootSystem(psiSoilLayers4,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS31 = hydraulics.supplyFunctionRootSystem(psiSoilLayers1,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS32 = hydraulics.supplyFunctionRootSystem(psiSoilLayers2,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS33 = hydraulics.supplyFunctionRootSystem(psiSoilLayers3,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
supplyRS34 = hydraulics.supplyFunctionRootSystem(psiSoilLayers4,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  maxNsteps = 400, psiMax = psiMax)
...
@
\end{center}
The derivative of $dE/d\Psi_{leaf}$ for the supply function of the root system is obtained numerically. An advantage of working with elements in parallel is that we can know the water potentials in different points of the root system. Here we plot them for the results of silt loam texture and the first and last soil potential vectors defined above:
\begin{center}
...
plot(supplyRS21$E, supplyRS21$Elayers[,1], type="l", col="black", xlab="Total flow rate", 
     ylab = "Path flow rate",  main="Soil potentials 1",  xlim=c(0, 4), ylim=c(-1,4), lwd=1)
lines(supplyRS21$E, supplyRS21$Elayers[,2], lty=2)
lines(supplyRS21$E, supplyRS21$Elayers[,3], lty=3)
abline(h=0, col="gray")
legend("topleft", legend=c("Layer 1", "Layer 2", "Layer 3"), bty="n", lty=c(1,2,3))
plot(supplyRS22$E, supplyRS22$Elayers[,1], type="l", col="black", xlab="Total flow rate", 
     ylab = "Path flow rate",  main="Soil potentials 2",  xlim=c(0, 2), ylim=c(-0.5,2), lwd=1)
lines(supplyRS22$E, supplyRS22$Elayers[,2], lty=2)
lines(supplyRS22$E, supplyRS22$Elayers[,3], lty=3)
abline(h=0, col="gray")
legend("topleft", legend=c("Layer 1", "Layer 2", "Layer 3"), bty="n", lty=c(1,2,3))
plot(supplyRS23$E, supplyRS23$Elayers[,1], type="l", col="black", xlab="Total flow rate", 
     ylab = "Path flow rate",  main="Soil potentials 3",  xlim=c(0, 0.4), ylim=c(-0.2,0.4), lwd=1)
lines(supplyRS23$E, supplyRS23$Elayers[,2], lty=2)
lines(supplyRS23$E, supplyRS23$Elayers[,3], lty=3)
abline(h=0, col="gray")
legend("topleft", legend=c("Layer 1", "Layer 2", "Layer 3"), bty="n", lty=c(1,2,3))
plot(supplyRS24$E, supplyRS24$Elayers[,1], type="l", col="black", xlab="Total flow rate", 
     ylab = "Path flow rate",  main="Soil potentials 4",  xlim=c(0, 0.05), ylim=c(-0.05,0.05), lwd=1)
lines(supplyRS24$E, supplyRS24$Elayers[,2], lty=2)
lines(supplyRS24$E, supplyRS24$Elayers[,3], lty=3)
abline(h=0, col="gray")
legend("topleft", legend=c("Layer 1", "Layer 2", "Layer 3"), bty="n", lty=c(1,2,3))
@
\end{center}
...
We can use a network of $(N \times 2 + 2)$ resistance elements to represent the soil-plant continuum, with soil being represented in $N$ different layers. As before, the $N$ soil layers are in parallel up to the root crown. From there there are two final elements: the stem xylem and the leaf. After calculating flows and potentials in the root system, the water potential at the upper end of the stem ($\Psi_{stem}$) is obtained integrating the stem xylem vulnerability curve, using $\Psi_{up,k}=\Psi_{rootcrown}$ and assuming a steady-state flow $E$. Leaf water potential ($\Psi_{leaf}$) is obtained integrating the leaf vulnerability curve, using $\Psi_{up,k}=\Psi_{stem}$ and assuming a steady-state flow $E$.The whole supply function $E(\Psi_{leaf}$) is obtained repeating these operations from $E=0$ to a critical value $E_{crit}$. 

The supply functions for each soil texture and starting from the different soil water potential configurations (see function \texttt{hydraulics.supplyFunctionNetwork()}): 
\begin{center}
<<fig=TRUE, width=8, height=3.5, echo=FALSE>>==
par(mar=c(4,4,2,1), mfrow=c(1,2))
psiMax = -7.0
supplyNetwork11 = hydraulics.supplyFunctionNetwork(psiSoilLayers1,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  ...
                                  psiCav=0.0,
                                  maxNsteps = 400, psiMax = psiMax)
supplyNetwork12 = hydraulics.supplyFunctionNetwork(psiSoilLayers2,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  psiCav=0.0,
                                  maxNsteps = 400, psiMax = psiMax)
supplyNetwork13 = hydraulics.supplyFunctionNetwork(psiSoilLayers3,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=0.0,
                                  psiMax = psiMax)
supplyNetwork14 = hydraulics.supplyFunctionNetwork(psiSoilLayers4,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=0.0,
                                  psiMax = psiMax)
supplyNetwork21 = hydraulics.supplyFunctionNetwork(psiSoilLayers1,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=0.0,
                                  psiMax = psiMax)
supplyNetwork22 = hydraulics.supplyFunctionNetwork(psiSoilLayers2,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=0.0,
                                  psiMax = psiMax)
supplyNetwork23 = hydraulics.supplyFunctionNetwork(psiSoilLayers3,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=0.0,
                                  psiMax = psiMax)
supplyNetwork24 = hydraulics.supplyFunctionNetwork(psiSoilLayers4,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=0.0,
                                  psiMax = psiMax)
supplyNetwork31 = hydraulics.supplyFunctionNetwork(psiSoilLayers1,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=0.0,
                                  psiMax = psiMax)
supplyNetwork32 = hydraulics.supplyFunctionNetwork(psiSoilLayers2,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=0.0,
                                  psiMax = psiMax)
supplyNetwork33 = hydraulics.supplyFunctionNetwork(psiSoilLayers3,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=0.0,
                                  psiMax = psiMax)
supplyNetwork34 = hydraulics.supplyFunctionNetwork(psiSoilLayers4,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
...
supplyNetwork11emb = hydraulics.supplyFunctionNetwork(psiSoilLayers1,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  psiCav=psiCav,
                                  maxNsteps = 400, psiMax = psiMax)
supplyNetwork12emb = hydraulics.supplyFunctionNetwork(psiSoilLayers2,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork13emb = hydraulics.supplyFunctionNetwork(psiSoilLayers3,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork14emb = hydraulics.supplyFunctionNetwork(psiSoilLayers4,
                                  krhizomaxvec1,rep(n1,3),rep(alpha1,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork21emb = hydraulics.supplyFunctionNetwork(psiSoilLayers1,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork22emb = hydraulics.supplyFunctionNetwork(psiSoilLayers2,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork23emb = hydraulics.supplyFunctionNetwork(psiSoilLayers3,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork24emb = hydraulics.supplyFunctionNetwork(psiSoilLayers4,
                                  krhizomaxvec2,rep(n2,3),rep(alpha2,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork31emb = hydraulics.supplyFunctionNetwork(psiSoilLayers1,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork32emb = hydraulics.supplyFunctionNetwork(psiSoilLayers2,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork33emb = hydraulics.supplyFunctionNetwork(psiSoilLayers3,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  maxNsteps = 400, psiCav=psiCav,
                                  psiMax = psiMax)
supplyNetwork34emb = hydraulics.supplyFunctionNetwork(psiSoilLayers4,
                                  krhizomaxvec3,rep(n3,3),rep(alpha3,3),
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
...
@
\end{center}
As with the two-element representation of the soil-plant continuum, the derivative of $dE/d\Psi_{leaf}$ for the network topology is obtained numerically:
\begin{center}
<<fig=TRUE, width=8, height=3.5, echo=FALSE>>==
par(mar=c(4,4,2,1), mfrow=c(1,2))
...
lines(supplyNetwork21$E,-supplyNetwork21$PsiStem, lty=2, lwd=2)
...
lines(supplyNetwork21emb$E,-supplyNetwork21emb$PsiStem, lty=2)
...
lines(supplyNetwork24$E,-supplyNetwork24$PsiStem, lty=2, lwd=2)
...
lines(supplyNetwork24emb$E,-supplyNetwork24emb$PsiStem, lty=2)
...
\subsection{Hydraulics with plant water compartments}
So far we have been concerned with the steady-state flows assuming there are no other sources of water than that of the soil. Water potential at the upper end of the stem ($\Psi_{stem}$) was obtained integrating the stem xylem vulnerability curve, using $\Psi_{up,k}=\Psi_{rootcrown}$ and making the result equal to the input flow from the root system. Now we will consider the effect of additional sources of water from stem tissues on the water supply function and, more generally, in plant hydraulics.

\begin{figure}[h!]
\includegraphics[width=\textwidth]{hydraulics.jpg}
\end{figure}

\subsubsection{Effects of cavitation on the supply function}
From one time step to the next, the steady-state flow will dictate different values of water potential along the stem. As water potentials decrease, cavitation may occur in the more distal parts of the stem, with the consequent loss of conductance. In order to track the decrease of sapwood conductance one has to consider segments within the stem. Within each sapwood segment, we assume that the proportion of conductance loss (PLC) in a given stem segment $i$ is related to the relative water content ($RWC$) in its conduit vessels (i.e. the water volume in conduit vessels with respect to the maximum water volume):
\begin{equation}
PLC_{i} = 1 - RWC_{apo,s}(\Psi_{cav,i})
\end{equation}
where $RWC_{apo,s}$ is the function of relative water content for apoplastic stem tissue. With this assumption one can relate changes in PLC derived from cavitation with decreases in water content in xylem conduits. For each segment $i$, when $PLC_i$ increases the associated change in water content is a source of water can be added to the transpirational stream (Martin-StPaul et al. 2017). More importantly, one can invert $RWC_{apo,s}$ for given segment $i$ to find its $\Psi_{cav,i}$, the water potential corresponding to the current degree of cavitation.

The supply function of a hydraulic network including stem segments and keeping track of PLC works as follows. For any $E$ and $\Psi_{rootcrown}$ pair of values derived from the supply function of the root system, and given a time step, process all stem segments as follows: 
\begin{enumerate}
\item{Take the steady-state downstream water potential of the previous segment ($\Psi_{down,i-1}$, or $\Psi_{rootcrown}$ when processing the first segment) as upstream water potential ($\Psi_{up,i}$).}
\item{Calculate the current water volume within the segment corresponding to the current level of embolism (PLC), and the corresponding $\Psi_{cav,i}$ by inverting $RWC_{apo,s}$.}
\item{Calculate the drop in stem water potential (i.e., find $\Psi_{down,i}$) occurring within the segment, while including $\Psi_{cav,i}$ in the calculation of the segment vulnerability curve. Note that the maximum segment conductance is equal to the whole-stem conductance multiplied by the number of segments, because of the segmentation.}
\item{Calculate the water volume within the segment, corresponding to the new steady-state upstream and downstream water potential values: 
\begin{equation}
V_{i} = V_{sapwood,i} \cdot f_{apo, s} \cdot \frac{RWC_{apo,s}(\Psi_{up,i})+ RWC_{apo,s}(\Psi_{down,i})}{2}
\end{equation}
where $V_{sapwood,i}$ is the maximum water volume in segment $i$, i.e. the fraction of $V_{sapwood}$ depending on the number of segments. If conduit refilling is not permitted, the calculated volume cannot be larger than the initial volume.
\item{Add the difference between the initial volume and the current volume (both volumes on a per leaf area basis), divided by the duration of the time step and expressed in mmol H$_2$O, to the value of $E$ for the next segment to be processed.}
\end{enumerate}

After all stem segments are processed, one can continue with the pressure drop due to the leaf resistance. We first define five segments, three values of $V_{sapwood}$, the initial level of PLC (0\%) for each segment, the fraction of sapwood corresponding to apoplastic tissue and the time step:
<<>>=
Vmax1 = 0.001 
Vmax2 = 0.005
Vmax3 = 0.01 
...
The following plot shows the supply function, i.e. downstream flow against downstream leaf pressure, for the unsegmented case (continuous line) and for a stem with the five segments and the three different values of $V_{sapwood}$ (function \texttt{supplyFunctionAboveground}, dashed and dotted lines):
\begin{center}
...
par(mar=c(4,4,2,1))
psiMax = -7.0
supplyAG11 = hydraulics.supplyFunctionAboveGround(supplyRS11$E, supplyRS11$PsiRoot,
                                                  PLC, RWCstorage,
                                                  kstemmax, stemc, stemd,
                                                  kleafmax, leafc, leafd,
                                                  Vmax1, fapo, pi0, epsilon,
                                                  klat, ksto, F, tstep, psiStep = -0.0001)
supplyAG12 = hydraulics.supplyFunctionAboveGround(supplyRS11$E, supplyRS11$PsiRoot,
                                                  PLC, RWCstorage,
                                                  kstemmax, stemc, stemd,
                                                  kleafmax, leafc, leafd,
                                                  Vmax2, fapo, pi0, epsilon,
                                                  klat, ksto, F, tstep, psiStep = -0.0001)
supplyAG13 = hydraulics.supplyFunctionAboveGround(supplyRS11$E, supplyRS11$PsiRoot,
                                                  PLC, RWCstorage,
                                                  kstemmax, stemc, stemd,
                                                  kleafmax, leafc, leafd,
                                                  Vmax3, fapo, pi0, epsilon,
                                                  klat, ksto, F, tstep, psiStep = -0.0001)
plot(-supplyNetwork11$PsiLeaf, supplyNetwork11$E, type="l", col=col1, ylab="Flow rate  (mmol·m-2·s-1)", 
     xlab = "Leaf pressure (-MPa)",  xlim=c(0, 6), ylim=c(0,7), lwd=1)
lines(-supplyAG11$PsiLeaf, supplyAG11$E, lty=2, col=col1)
lines(-supplyAG12$PsiLeaf, supplyAG12$E, lty=3, col=col1)
lines(-supplyAG13$PsiLeaf, supplyAG13$E, lty=4, col=col1)
plot(-supplyNetwork11$PsiLeaf, supplyNetwork11$dEdP, type="l", col=col1, ylab="dEdP", 
     xlab = "Leaf pressure (-MPa)",  xlim=c(0, 6), ylim =c(0, 3), lwd=1)
lines(-supplyAG11$PsiLeaf, supplyAG11$dEdP, lty=2, col=col1)
lines(-supplyAG12$PsiLeaf, supplyAG12$dEdP, lty=3, col=col1)
lines(-supplyAG13$PsiLeaf, supplyAG13$dEdP, lty=4, col=col1)
@
\end{center}
As expected, the larger the sapwood volume, the larger the effect of conduit cavitation on the downstream flow. The observed initial increase in $dE_i/d\Psi$ may be due to the non-linear water release. In a dynamic context with sufficiently small time steps, the PLC from the previous time step should not be too different to the new PLC value, so that such increases should be rather small.


\subsubsection{Symplastic storage compartment}
 Additionally, the symplastic storage compartment contribute to sapflow dynamics by continuously exchanging water with xylem. If we consider the two sources, the downstream instantaneous flow may not necessarily be equal to the flow entering the stem:
\begin{equation}
E_{stem} = E_{root} - (\Delta{S_{sapwood}}/\Delta{t}) - (\Delta{S_{storage}}/\Delta{t})
\end{equation}
where $E_{stem}$ is the stem outflow, $E_{root}$ is the input flow (from the root system), $\Delta{S_{sapwood}}/\Delta{t}$ is the change in water of the apoplastic (sapwood) tissue and  $\Delta{S_{storage}}/\Delta{t}$ is the change in water of the symplastic storage compartment. The amount of apoplastic water to be added or subtracted from the input flow is simply calculated from changes in sapwood water derived from changes in the steady-state stem water potential between consecutive time steps:
\begin{equation}
\frac{\Delta{S_{sapwood}}}{\Delta{t}} = \frac{S_{sapwood}(\Psi_{stem, t}) - S_{sapwood}(\Psi_{stem, t-1})}{\Delta{t}}
\end{equation}
If $\Delta{S_{storage}}/\Delta{t}$ is negative, then there is a flow from the storage compartment to the xylem. Otherwise, flow is from the xylem to the storage compartment. Here we assume that the storage compartment has a common water potential $\Psi_{storage}$ and the flow between the compartment at any height of the stem depends on the difference in water potential between the compartment and the xylem:
\begin{equation}
\frac{\Delta{S_{storage}}}{\Delta{t}} = \int_{\Psi_{stem}}^{\Psi_{rootcrown}}{f_{storage}(\Psi) d\Psi}
\end{equation}
where $f_{storage}$ is the exchange flow between xylem and storage compartment for a given $\Psi$ value of the xylem. Since exchange of water occurs along the cylinder surrounding stem xylem, the second integral has to be re-expressed as a function of height ($h$):
\begin{equation}
\int_{\Psi_{up}}^{\Psi_{down}}{f_{storage}(\Psi) d\Psi} = \int_{0}^{H}{f_{storage}(\Psi_h) dh}
\end{equation}
where the exchange flow at any height $h$ is:
\begin{equation}
f_{storage}(\Psi_h) = (\Psi_h - \Psi_{storage})\cdot 2 \pi \cdot D_{int} \cdot L
\end{equation}
where $D_{int}$ is the internal radius separating sapwood from cambial cells, $L$ is the radial conductivity (in m·MPa$^{-1}$·s$^{-1}$) and $H$ is the height of the stem (plant). If we assume a linear change of water potential along stem height, we obtain the following expression for the instantaneous (in m$^3$·s$^{-1}$) storage effect on flow:

\begin{equation}
\Delta{S}/\Delta{t} = 2 \pi \cdot D_{int} \cdot L \cdot H \cdot \left[ \frac{\Psi_{stem}-\Psi_{rootcrown}}{2} - \Psi_{storage}\right]}
\end{equation}
Note that the storage effect needs to be reexpressed in flow per leaf area unit, i.e. mmol·m$^{-2}$·s$^{-1}$.



\subsection{Parameterization of hydraulics}
...
While leaf maximum hydraulic conductance ($k_{l, max}$, in mmol·m$^{-2}$·s$^{-1}$·MPa$^{-1}$) is often known for several species (if missing \texttt{medfate} assumes $k_{l, max}=6$ for conifers and $k_{l, max}=8$ for angiosperms [Sack \emph{et al.} 2006]), aboveground and belowground stem maximum conductance values at the plant level ($k_{s, max}$ and $k_{r, max}$) will not be normally available and the same for the rhizosphere ($k_{rh, max}$). \texttt{medfate} provides routines to estimate them from a minimum set of input parameters. The whole process of estimation of those parameters is done automatically in functions \texttt{swbInput()} and \texttt{forest2swbInput()}, with the user controlling the process through parameter \texttt{control} (see function \texttt{defaultControl()} for control parameter defaults). In the following we detail the calculations and present individual functions that perform partial calculations.

\subsubsection{Stem xylem maximum conductance}
Estimation of maximum stem conductance ($k_{s,max}$, in mmol·m$^{-2}$·s$^{-1}$·MPa$^{-1}$) is done by function \texttt{hydraulics.maximumStemHydraulicConductance()} and follows the work by Savage \emph{et al.} (2010), Olson \emph{et al.} (2014) and Christoffersen \emph{et al.} (2016). Calculations are based on tree height and two species-specific parameters: maximum sapwood reference conductivity ($K_{s,max,ref}$, in kg·m$^{-1}$·s$^{-1}$·MPa$^{-1}$;  see parameter \texttt{xylem\_kmax} in \texttt{SpParamsMED}) and the ratio of leaf area to sapwood area ($A_{l}/A_{s}$; \texttt{Al2As} in \texttt{SpParamsMED}), i.e. the inverse of the Huber value $H_v$.
...
H = 400 # in cm
Href = 660 # in cm
f = hydraulics.referenceConductivityHeightFactor(Href, H);
f
...
kstemmax = hydraulics.maximumStemHydraulicConductance(xylem_kmax, 
...
<<>>=
hydraulics.terminalConduitRadius(100.0)
@
...
for(i in 1:length(Hs)) tapnotap[i] = hydraulics.taperFactorSavage(Hs[i])
...
<<>>==
kstemmax_tap = hydraulics.maximumStemHydraulicConductance(xylem_kmax, 
...
for (i in 1:length(Hs)) {
  qi_with_tap[i] <- hydraulics.maximumStemHydraulicConductance(
  xylem_kmax, Href, Al2As, Hs[i], angiosperm = TRUE, taper =TRUE
)
  qi_no_tap[i] <- hydraulics.maximumStemHydraulicConductance(
...
Z50 = 200
Z95 = 1500
v1 = root.ldrDistribution(Z50,Z95, d)
v1
@
Having this information, the calculation of root length (i.e. the sum of vertical and radial lengths) to each layer ($L_j$) is done using function \texttt{root.rootLength()}:
<<>>==
rl = root.rootLengths(v1, d)
rl
@
where lengths are in mm. The proportion of total root xylem conductance corresponding to each layer ($w_j$) is given by \texttt{root.xylemConductanceProportions()}:
<<>>==
w1 = root.xylemConductanceProportions(v1, d)
w1
@
...
\end{equation}
where $w_j$ are root xylem conductance proportion of layer $j$ and $L_j$ is the root length (in m) to layer $j$. Note that here we use weights $w_j$ assuming they represent proportions of total sapwood area that come from each layer (i.e. the longer the path the larger the proportion of sapwood area). This calculation is made available by function \texttt{hydraulics.maximumRootHydraulicConductance()}. Let's consider the same \emph{Q. ilex} tree of 4m height as before. If we specify root xylem specific conductivity as $K_{r,max,ref} = K_{s,max,ref} =0.77$ we have:
<<>>=
rootxylem_kmax = 0.77
krootmax = hydraulics.maximumRootHydraulicConductance(rootxylem_kmax, Al2As, 
                                                      v1, d)
krootmax
@
...
for (i in 1:length(Hs)) {
  ks <- hydraulics.maximumStemHydraulicConductance(xylem_kmax, Href, Al2As, Hs[i], taper = FALSE)
...
for(i in 1:length(pres)) {
  krhizomaxvec1[1]= hydraulics.findRhizosphereMaximumConductance(pres[i], 
                     s$VG_n[1],s$VG_alpha[1],
                     krootmax, rootc,rootd, 
                     kstemmax, stemc, stemd,
                     kleafmax, leafc, leafd)
  krhizomaxvec1[2]= hydraulics.findRhizosphereMaximumConductance(pres[i], 
                     s$VG_n[2],s$VG_alpha[2],
                     krootmax, rootc,rootd, 
                     kstemmax, stemc, stemd,
                     kleafmax, leafc, leafd)
  krhizomaxvec1[3]= hydraulics.findRhizosphereMaximumConductance(pres[i], 
                     s$VG_n[3],s$VG_alpha[3],
                     krootmax, rootc,rootd, 
                     kstemmax, stemc, stemd,
                     kleafmax, leafc, leafd)
  sfn1 = hydraulics.supplyFunctionNetwork(psiSoilLayers1,
                                  ...
                                  psiCav=psiCav,
                                  maxNsteps = 400, psiMax = psiMax)
  sfn2 = hydraulics.supplyFunctionNetwork(psiSoilLayers2,
                                  krhizomaxvec1,s$VG_n,s$VG_alpha,
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc, leafd,
                                  psiCav=psiCav,
                                  maxNsteps = 400, psiMax = psiMax)
  sfn3 = hydraulics.supplyFunctionNetwork(psiSoilLayers3,
                                  krhizomaxvec1,s$VG_n,s$VG_alpha,
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc, leafd,
                                  psiCav=psiCav,
                                  maxNsteps = 400, psiMax = psiMax)
  sfn4 = hydraulics.supplyFunctionNetwork(psiSoilLayers4,
                                  krhizomaxvec1,s$VG_n,s$VG_alpha,
                                  krootmaxvec, rootc,rootd,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc, leafd,
...
<<>>=
krmax = rep(0,3) 
krmax[1]= hydraulics.findRhizosphereMaximumConductance(15, 
                     s$VG_n[1],s$VG_alpha[1],
                     krootmax, rootc,rootd, 
                     kstemmax, stemc, stemd,
                     kleafmax, leafc, leafd)
krmax[2] = hydraulics.findRhizosphereMaximumConductance(15, 
                      s$VG_n[2],s$VG_alpha[2],
                      krootmax, rootc,rootd, 
                      kstemmax, stemc, stemd,
                      kleafmax, leafc, leafd)
krmax[3] = hydraulics.findRhizosphereMaximumConductance(15, 
                      s$VG_n[3],s$VG_alpha[3],
                      krootmax, rootc,rootd, 
                      kstemmax, stemc, stemd,
                      kleafmax, leafc, leafd)
...
\subsubsection{Sapwood water storage capacity}
The water storage capacity of sapwood tissue per leaf area unit ($V_{sapwood}$; in m$^3$·m$^{-2}$) can be estimated as the product of stem height ($H$ in m) and Huber value ($H_v$; ratio of sapwood area to leaf area in m$^2$·m$^{-2}$):
\begin{equation}
V_{sapwood} = H \cdot H_v \cdot \Theta_{sapwood}
\end{equation}
$\Theta_{sapwood}$ is sapwood porosity (cm$^3$ of water per cm$^3$ of tissue), which can be estimated from wood density ($\rho_{wood}$; in g·cm$^{-3}$):
\begin{equation}
\Theta_{sapwood} = 1 - (\rho_{wood} / 1.54)
\end{equation}
where the density of wood substance can be assumed to be fixed and equal to 1.54 g·cm$^{-3}$ (Dunlap 1914). For example, wood densities ranging from 0.443 to 1.000 g·cm$^{-3}$ result in sapwood porosity values between 0.35 and 0.71. Parameters $\pi_{0,s}$ and $\epsilon_{s}$ for the pressure-volume of sapwood symplastic tissue and the apoplastic fraction in xylem $f_{apo, s}$, should be specified on a species basis.

For example, let's calculate the stem water capacity per leaf area in a stem of a Q. ilex tree of 15 m height:
<<>>==
wd = 1.0
Al2As = 2512 
H = 1500 # 15 m
Vsapwood = hydraulics.maximumStemWaterCapacity(Al2As, H, wd)
Vsapwood
@ 

\section{Leaf VPD, conductance to water vapor and photosynthesis}
...
\subsection{Leaf temperature and vapor pressure deficit}
...
The following figures illustrate the value of $T_{leaf}$ for two leaf sizes and varying values of wind speed and flow rate, calculated for 24ºC canopy temperature and 740 W·m$^{-2}$ instantaneous absorved radiation (see function \texttt{biophysics.leafTemperature}):
...
for(i in 1:length(uvec)) {
  for(j in 1:length(E)){
    LT1[i,j] = biophysics.leafTemperature(Rabs,Tcan, uvec[i], E[j], 10.0)
    LT2[i,j] = biophysics.leafTemperature(Rabs,Tcan, uvec[i], E[j], 0.1)
  }
} 
...
u = 2 # m*s-1
par(mar=c(4,4,2,1), mfrow=c(1,2))
psi2A11 = photo.leafPhotosynthesisFunction(supplyNetwork11,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A12 = photo.leafPhotosynthesisFunction(supplyNetwork12,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A13 = photo.leafPhotosynthesisFunction(supplyNetwork13,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A14 = photo.leafPhotosynthesisFunction(supplyNetwork14,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A21 = photo.leafPhotosynthesisFunction(supplyNetwork21,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A22 = photo.leafPhotosynthesisFunction(supplyNetwork22,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A23 = photo.leafPhotosynthesisFunction(supplyNetwork23,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A24 = photo.leafPhotosynthesisFunction(supplyNetwork24,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A31 = photo.leafPhotosynthesisFunction(supplyNetwork31,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A32 = photo.leafPhotosynthesisFunction(supplyNetwork32,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A33 = photo.leafPhotosynthesisFunction(supplyNetwork33,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A34 = photo.leafPhotosynthesisFunction(supplyNetwork34,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)

psi2A11emb = photo.leafPhotosynthesisFunction(supplyNetwork11emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A12emb = photo.leafPhotosynthesisFunction(supplyNetwork12emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A13emb = photo.leafPhotosynthesisFunction(supplyNetwork13emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A14emb = photo.leafPhotosynthesisFunction(supplyNetwork14emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A21emb = photo.leafPhotosynthesisFunction(supplyNetwork21emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A22emb = photo.leafPhotosynthesisFunction(supplyNetwork22emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A23emb = photo.leafPhotosynthesisFunction(supplyNetwork23emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A24emb = photo.leafPhotosynthesisFunction(supplyNetwork24emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A31emb = photo.leafPhotosynthesisFunction(supplyNetwork31emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A32emb = photo.leafPhotosynthesisFunction(supplyNetwork32emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A33emb = photo.leafPhotosynthesisFunction(supplyNetwork33emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
psi2A34emb = photo.leafPhotosynthesisFunction(supplyNetwork34emb,Catm, Patm,Tcan, vpa, u, Rabs, Q, Vmax298, Jmax298, Gmin, Gmax)
...
\subsection{Leaf conductance to water vapor}
...
\end{equation}
\subsection{Crown photosynthesis}
...
Multilayer canopy models allow evaluating leaf conditions, stomatal conductance and photosynthesis for different points of the canopy. However, this comes at high computational cost. While big-leaf canopy models are known to be unaccurate under some situations, sun-shade canopy models (De Pury and Farquhar 1997) provide estimates that are close to multiple layer models (Hikosaka 2016). Sun-shade models involve: (a) aggregating the leaf area of sunlit/shade leaves across layers; (b) aggregating the light absorbed by leaves of each kind across layers; and (c) aggregating maximum assimilation rates across layers, again separating sunlit and shade leaves. One then calls the photosynthesis model twice (i.e. once for shade leaves and once for sunlit leaves), using the aggregated maximum assimilation rates. Separating the two kinds of leaves acknowledges that they operate at different parts of the light-saturation curve. The following figure provides the canopy photosynthesis functions obtained, under different situations, using a full 10-layer canopy description (top), a sunshade canopy model (center) or a big-leaf model (bottom). These were generated using functions \texttt{photo.multilayerPhotosynthesisFunction()}, \texttt{photo.sunshadePhotosynthesisFunction()} and \texttt{photo.leafPhotosynthesisFunction()}, respectively. Note the coincidence between the multi-layer and the sun-shade models.
<<echo=FALSE>>=
psi2A11can = photo.multilayerPhotosynthesisFunction(supplyNetwork11,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A12can = photo.multilayerPhotosynthesisFunction(supplyNetwork12,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A13can = photo.multilayerPhotosynthesisFunction(supplyNetwork13,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A14can = photo.multilayerPhotosynthesisFunction(supplyNetwork14,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A21can = photo.multilayerPhotosynthesisFunction(supplyNetwork21,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A22can = photo.multilayerPhotosynthesisFunction(supplyNetwork22,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A23can = photo.multilayerPhotosynthesisFunction(supplyNetwork23,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A24can = photo.multilayerPhotosynthesisFunction(supplyNetwork24,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A31can = photo.multilayerPhotosynthesisFunction(supplyNetwork31,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A32can = photo.multilayerPhotosynthesisFunction(supplyNetwork32,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A33can = photo.multilayerPhotosynthesisFunction(supplyNetwork33,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A34can = photo.multilayerPhotosynthesisFunction(supplyNetwork34,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)

psi2A11canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork11emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A12canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork12emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A13canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork13emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A14canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork14emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A21canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork21emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A22canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork22emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A23canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork23emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A24canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork24emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A31canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork31emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A32canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork32emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A33canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork33emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
psi2A34canemb = photo.multilayerPhotosynthesisFunction(supplyNetwork34emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298layer, Jmax298layer, Gmin, Gmax)
...
psi2A11ss = photo.sunshadePhotosynthesisFunction(supplyNetwork11,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A12ss = photo.sunshadePhotosynthesisFunction(supplyNetwork12,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A13ss = photo.sunshadePhotosynthesisFunction(supplyNetwork13,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A14ss = photo.sunshadePhotosynthesisFunction(supplyNetwork14,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A21ss = photo.sunshadePhotosynthesisFunction(supplyNetwork21,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A22ss = photo.sunshadePhotosynthesisFunction(supplyNetwork22,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A23ss = photo.sunshadePhotosynthesisFunction(supplyNetwork23,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A24ss = photo.sunshadePhotosynthesisFunction(supplyNetwork24,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A31ss = photo.sunshadePhotosynthesisFunction(supplyNetwork31,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A32ss = photo.sunshadePhotosynthesisFunction(supplyNetwork32,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A33ss = photo.sunshadePhotosynthesisFunction(supplyNetwork33,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A34ss = photo.sunshadePhotosynthesisFunction(supplyNetwork34,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A11ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork11emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A12ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork12emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A13ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork13emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A14ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork14emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A21ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork21emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A22ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork22emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A23ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork23emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A24ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork24emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A31ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork31emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A32ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork32emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A33ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork33emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
psi2A34ssemb = photo.sunshadePhotosynthesisFunction(supplyNetwork34emb,Catm, Patm,Tcan, vpa, 
                                                SLarea, SHarea, ulayer, 
                                                absRadSL, absRadSH, QSL, QSH,
                                                Vmax298SL, Vmax298SH, Jmax298SL, Jmax298SH, Gmin, Gmax)
...
Gmax = 2.0
psi2A11bl = photo.leafPhotosynthesisFunction(supplyNetwork11,Catm, Patm,Tcan, vpa, ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A12bl = photo.leafPhotosynthesisFunction(supplyNetwork12,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A13bl = photo.leafPhotosynthesisFunction(supplyNetwork13,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A14bl = photo.leafPhotosynthesisFunction(supplyNetwork14,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A21bl = photo.leafPhotosynthesisFunction(supplyNetwork21,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A22bl = photo.leafPhotosynthesisFunction(supplyNetwork22,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A23bl = photo.leafPhotosynthesisFunction(supplyNetwork23,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A24bl = photo.leafPhotosynthesisFunction(supplyNetwork24,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A31bl = photo.leafPhotosynthesisFunction(supplyNetwork31,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A32bl = photo.leafPhotosynthesisFunction(supplyNetwork32,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A33bl = photo.leafPhotosynthesisFunction(supplyNetwork33,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A34bl = photo.leafPhotosynthesisFunction(supplyNetwork34,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)

psi2A11blemb = photo.leafPhotosynthesisFunction(supplyNetwork11emb,Catm, Patm,Tcan, vpa, ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A12blemb = photo.leafPhotosynthesisFunction(supplyNetwork12emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A13blemb = photo.leafPhotosynthesisFunction(supplyNetwork13emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A14blemb = photo.leafPhotosynthesisFunction(supplyNetwork14emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A21blemb = photo.leafPhotosynthesisFunction(supplyNetwork21emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A22blemb = photo.leafPhotosynthesisFunction(supplyNetwork22emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A23blemb = photo.leafPhotosynthesisFunction(supplyNetwork23emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A24blemb = photo.leafPhotosynthesisFunction(supplyNetwork24emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A31blemb = photo.leafPhotosynthesisFunction(supplyNetwork31emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A32blemb = photo.leafPhotosynthesisFunction(supplyNetwork32emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A33blemb = photo.leafPhotosynthesisFunction(supplyNetwork33emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
psi2A34blemb = photo.leafPhotosynthesisFunction(supplyNetwork34emb,Catm, Patm,Tcan, vpa,  ulayer,
                                             absRadBL,QBL,Vmax298BL, Jmax298BL, Gmin, Gmax, 1.0, BLarea)
Gmax = 0.5
@
\begin{center}
...
Sperry et al (2016b) presented a cost-benefit approach where hydraulic costs of opening the stomata are compared against photosynthetic gain. Details of their approach, and two suggested variants, a given in the next two subsections. The final subsection explains how to scale stomatal regulation (and hence, transpiration and photosynthesis) from leaf to plant.
 ...
 \subsection{Cost and gain functions}
...
\begin{eqnarray}
\theta_2(\Psi_{leaf}) &=& \frac{k_{l, max}-k_l(\Psi_{leaf})}{k_{l,max}}\\
\end{eqnarray}
where $k_l$ is the leaf conductance function; and and $k_{l,max}$ is the maximum leaf conductance value. Using the leaf vulnerability curve for the cost function is grounded on the fact that stomatal regulation occurs at leaves, so that instantaneous regulation should respond to the loss of hydraulic conductance at this point, independently of what happens to the rest of the continuum. Hormonal signals from root to leaf are assumed to regulate stomatal aperture at longer time scales. Obviously, $\theta_2$ is the same before irreversible cavitation. The difference between them may be interpreted as the following. $\theta_2$ strictly follows the potential at the leaf level (and hence could be related to a loss of turgor). 

<<echo=FALSE>>==
PM11 =transp.profitMaximization(supplyNetwork11, psi2A11, type=1, Gmax)
PM12 =transp.profitMaximization(supplyNetwork12, psi2A12, type=1, Gmax)
PM13 =transp.profitMaximization(supplyNetwork13, psi2A13, type=1, Gmax)
PM14 =transp.profitMaximization(supplyNetwork14, psi2A14, type=1, Gmax)
PM21 =transp.profitMaximization(supplyNetwork21, psi2A21, type=1, Gmax)
PM22 =transp.profitMaximization(supplyNetwork22, psi2A22, type=1, Gmax)
PM23 =transp.profitMaximization(supplyNetwork23, psi2A23, type=1, Gmax)
PM24 =transp.profitMaximization(supplyNetwork24, psi2A24, type=1, Gmax)
PM31 =transp.profitMaximization(supplyNetwork31, psi2A31, type=1, Gmax)
PM32 =transp.profitMaximization(supplyNetwork32, psi2A32, type=1, Gmax)
PM33 =transp.profitMaximization(supplyNetwork33, psi2A33, type=1, Gmax)
PM34 =transp.profitMaximization(supplyNetwork34, psi2A34, type=1, Gmax)

PM11emb =transp.profitMaximization(supplyNetwork11emb, psi2A11emb, type=1, Gmax)
PM12emb =transp.profitMaximization(supplyNetwork12emb, psi2A12emb, type=1, Gmax)
PM13emb =transp.profitMaximization(supplyNetwork13emb, psi2A13emb, type=1, Gmax)
PM14emb =transp.profitMaximization(supplyNetwork14emb, psi2A14emb, type=1, Gmax)
PM21emb =transp.profitMaximization(supplyNetwork21emb, psi2A21emb, type=1, Gmax)
PM22emb =transp.profitMaximization(supplyNetwork22emb, psi2A22emb, type=1, Gmax)
PM23emb =transp.profitMaximization(supplyNetwork23emb, psi2A23emb, type=1, Gmax)
PM24emb =transp.profitMaximization(supplyNetwork24emb, psi2A24emb, type=1, Gmax)
PM31emb =transp.profitMaximization(supplyNetwork31emb, psi2A31emb, type=1, Gmax)
PM32emb =transp.profitMaximization(supplyNetwork32emb, psi2A32emb, type=1, Gmax)
PM33emb =transp.profitMaximization(supplyNetwork33emb, psi2A33emb, type=1, Gmax)
PM34emb =transp.profitMaximization(supplyNetwork34emb, psi2A34emb, type=1, Gmax)

PM11_s2 =transp.profitMaximization(supplyNetwork11, psi2A11, type=2, Gmax, kleafmax)
PM12_s2 =transp.profitMaximization(supplyNetwork12, psi2A12,type=2, Gmax,  kleafmax)
PM13_s2 =transp.profitMaximization(supplyNetwork13, psi2A13, type=2, Gmax, kleafmax)
PM14_s2 =transp.profitMaximization(supplyNetwork14, psi2A14,type=2, Gmax,  kleafmax)
PM21_s2 =transp.profitMaximization(supplyNetwork21, psi2A21,type=2, Gmax,  kleafmax)
PM22_s2 =transp.profitMaximization(supplyNetwork22, psi2A22,type=2, Gmax,  kleafmax)
PM23_s2 =transp.profitMaximization(supplyNetwork23, psi2A23,type=2, Gmax,  kleafmax)
PM24_s2 =transp.profitMaximization(supplyNetwork24, psi2A24,type=2, Gmax,  kleafmax)
PM31_s2 =transp.profitMaximization(supplyNetwork31, psi2A31, type=2, Gmax, kleafmax)
PM32_s2 =transp.profitMaximization(supplyNetwork32, psi2A32,type=2, Gmax,  kleafmax)
PM33_s2 =transp.profitMaximization(supplyNetwork33, psi2A33, type=2, Gmax, kleafmax)
PM34_s2 =transp.profitMaximization(supplyNetwork34, psi2A34, type=2, Gmax, kleafmax)

PM11emb_s2 =transp.profitMaximization(supplyNetwork11emb, psi2A11emb, type=2, Gmax, kleafmax)
PM12emb_s2 =transp.profitMaximization(supplyNetwork12emb, psi2A12emb, type=2, Gmax, kleafmax)
PM13emb_s2 =transp.profitMaximization(supplyNetwork13emb, psi2A13emb, type=2, Gmax, kleafmax)
PM14emb_s2 =transp.profitMaximization(supplyNetwork14emb, psi2A14emb, type=2, Gmax, kleafmax)
PM21emb_s2 =transp.profitMaximization(supplyNetwork21emb, psi2A21emb, type=2, Gmax, kleafmax)
PM22emb_s2 =transp.profitMaximization(supplyNetwork22emb, psi2A22emb, type=2, Gmax, kleafmax)
PM23emb_s2 =transp.profitMaximization(supplyNetwork23emb, psi2A23emb, type=2, Gmax, kleafmax)
PM24emb_s2 =transp.profitMaximization(supplyNetwork24emb, psi2A24emb, type=2, Gmax, kleafmax)
PM31emb_s2 =transp.profitMaximization(supplyNetwork31emb, psi2A31emb, type=2, Gmax, kleafmax)
PM32emb_s2 =transp.profitMaximization(supplyNetwork32emb, psi2A32emb, type=2, Gmax, kleafmax)
PM33emb_s2 =transp.profitMaximization(supplyNetwork33emb, psi2A33emb, type=2, Gmax, kleafmax)
PM34emb_s2 =transp.profitMaximization(supplyNetwork34emb, psi2A34emb, type=2, Gmax, kleafmax)
@
...
\subsection{Profit maximization at the leaf level}
...
plot(-supplyNetwork11$PsiLeaf, PM11_s2$Profit, type="l", col=col1, ylab="Profit 2", xlab = "Leaf sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(-1,1), lwd=1, main="original")
...
plot(-supplyNetwork11emb$PsiLeaf, PM11emb_s2$Profit, type="l", col=col1, ylab="Profit 2", xlab = "Leaf sap pressure (-MPa)",  xlim=c(0, 7), ylim=c(-1,1), lwd=1, main="after cavitation")
...
\subsection{Scaling stomatal regulation to the plant level}
...
\begin{center}
    ...
    k_{l, max} & mmol·s$^{-1}$ ·m$^{-2}$·MPa$^{-1}$ &  & Maximum leaf conductance (per leaf area unit). \\ \hline
    ...
    \rho_{wood} & g·cm$^{-3}$ & \texttt{WoodDens} & Wood density. \\ \hline
