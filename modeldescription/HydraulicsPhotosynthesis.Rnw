...
So far we have been concerned with the steady-state flows assuming there are no other sources of water than that of the soil. Now we will consider the effect of additional sources of water from leaf and stem tissues on the water supply function and, more generally, in plant hydraulics. Considering water compartments allow tracking leaf and stem (i.e. plant) dessication, either when the plant is connected to the soil or when transpiration flow comes from the plant itself.

In the previous section, we described how the supply function of the network representing the continuum is build for a given state of soil water potentials. This approach allows determining flows and water potential for \emph{steady-state} conditions. In a dynamic context, after building the supply function one has to determine stomatal conductance (see sections devoted to photosynthesis and stomatal regulation) to decide on a value of $E$ and the corresponding steady-state configuration. Because light and temperature conditions change along the day, stomatal conductance has to be determined for subdaily steps (e.g. 1-hour steps) and the flows added. At the end of the day, one may substract water from soil layers and recalculate the supply function for the next day. 

When considering plant compartments, one may allow flow from compartments to affect the supply function. However, since the supply function is concerned with instantaneous flows, the instantaneous change in water content of compartments is very small, and one may assume that the contribution of plant compartments to the shape of the supply function is negligible. We will take this assumption here and hence, assume that considering plant compartments does not change the determination of steady-state stomatal conductance (and the water extracted from the soil) corresponding to any given subdaily step. However, the \emph{total} amount of water transpired by plants between two consecutive subdaily steps (e.g. during 1 hour), or in a full day, will surely be affected by transient changes in plant water content. Therefore, changes in water compartments can be used to decide on the amount of water transpired between consecutive time steps.
