...
v &=& [(\alpha \Psi)^{n}+1]^{-1}
\end{eqnarray}
where $k_{rh,max}$ is the maximum rhizosphere conductance, and $n$ and $\alpha$ are texture-specific parameters (see Leij et al. 1996; Carsel \& Parrish 1988). These are automatically set by function \texttt{soil()} when initializing soil objects (see parameters \texttt{VG\_alpha} and \texttt{VG\_n} in the output of \texttt{soil()}), but we can use function \texttt{soil.vanGenuchtenParams()} to derive them from texture types:
<<>>=
textures = c("Sandy loam","Silt loam", "Clay")
#Textural parameters
#Sandy clay loam 
p1 = soil.vanGenuchtenParams(textures[1])
p1
alpha1 = p1[1]  
n1 = p1[2]
#Silt loam
p2 = soil.vanGenuchtenParams(textures[2])
p2
alpha2 =  p2[1]
n2 = p2[2]
#Silty clay
p3 = soil.vanGenuchtenParams(textures[3])
p3
alpha3 =  p3[1]
n3 = p3[2]
@
...
for(i in 1:length(psi)) rwc_s[i] = biophysics.symplasticRelativeWaterContent(psi[i],-3,12)
...
\begin{equation}
RWC_{apo}(\Psi_{stem}) = \frac{k_x(\Psi_{stem})}{k_{x,max}}
\end{equation}
The second reservoir is formed by more elastic living cells in the stem and can potentially be a larger source of water during relatively high water potentials. This source can be described using the relative water content of the symplasm, as done for leaves. By knowing the proportion of apoplastic tissue in the stem, $a_{f,s}$, we can obtain the overall relative water content in the stem as:
\begin{equation}
