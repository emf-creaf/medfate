...
Parameters of the pressure-volume curve (i.e. $\pi_{0,stem}$ and $\epsilon_{stem}$ for stem symplastic tissue are required for each species. When missing, \texttt{medfate} estimates them from wood density following Christophersen et al (2016):
\begin{equation}
...
Maximum stomatal conductance ($g_{swmax}$) is an input parameter for each species. When species-specific values are missing, the following relation with maximum hydraulic conductance ($k_{l, max}$) is used (Mencuccini 2003):
\begin{equation}
...
Rubisco's maximum carboxylation rate at 25ºC ($V_{max, 298}$, in $\mu$mol CO$_2$·s$^{-1}$·m$^{-2}$) is a required input parameter for each species (\texttt{Vmax298}). The maximum rate of electron transport at the same temperature ($J_{max, 298}$) is estimated from $V_{max, 298}$ using (Walker et al. 2014):
...
\begin{center}
    \begin{tabular}{| l | p{2cm} | l | p{5cm} |}
        ...
        $J_{max, 298}$ & $\mu$mol electrons·m$^{-2}$·s$^{-1}$ & & Maximum rate of electron transport at 298K \\ \hline
