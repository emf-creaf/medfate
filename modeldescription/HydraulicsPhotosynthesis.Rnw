...
A xylem element is assigned a two-parameter Weibull function as the vulnerability curve $k_x(\Psi)$:
\begin{equation}\label{eq:xylemvulnerability}
k_x(\Psi) = k_{x,max}\cdot e^{-((\Psi/d)^c)}
\end{equation}
where $k_{x,max}$ is the xylem maximum hydraulic conductance (defined as flow per surface unit and per pressure drop), and $c$ and $d$ are species-specific parameters (see parameters \texttt{VCstem\_c} and \texttt{VCstem\_d} in \texttt{SpParamsMED}). Note that parameter $d$ is the water potential (in MPa) at which $k_{x}(\Psi)/k_{x,max} = e^{-1} = 0.367$. Parameter $c$ controls the shape of the vulnerability curve (‘exponential’ shape with no threshold has $c \leq 1$, sigmoidal threshold has $c > 1$). For example, we define the following parameter values for a stem xylem ($k_{s,max}$):
<<>>==
kstemmax = 5.0 # in mmol·m-2·s-1·MPa-1
stemc = 3 
stemd = -3.0 # in MPa
@
For root xylem ($k_{r,max}$), we may assume a higher conductance (i.e. higher efficiency) but also higher vulnerability to cavitation (see parameters \texttt{VCroot\_c} and \texttt{VCroot\_d} in \texttt{SpParamsMED}):
<<>>==
krootmax = 6.6
rootc = 2
rootd = -2.5 #MPa
...
\end{equation}
where $k_{l,max}$ is the leaf maximum hydraulic conductance (see parameter \texttt{VCleaf\_kmax} in \texttt{SpParamsMED}). Values defined below specify higher conductance for leaves but also slightly higher vulnerability (see parameters \texttt{VCleaf\_c} and \texttt{VCleaf\_d} in \texttt{SpParamsMED}):
<<>>==
kleafmax = 10
leafc = 2
leafd = -2
...
\subsection{Changes in water content}
...
The overall relative water content of leaves can be obtained from $\Psi_{leaf}$ by calculating  $RWC_{sym}(\Psi_{leaf})$ and $RWC_{apo}(\Psi_{leaf})$ followed by assuming a constant apoplastic fraction $a_{f,l}$:
\begin{equation}
RWC(\Psi_{leaf}) = RWC_{apo}(\Psi_{leaf}) \cdot a_{f,l} + RWC_{sym}(\Psi_{leaf}) \cdot (1 - a_{f,l})
\end{equation}
...
Following Martin-StPaul et al. (2017), we consider two sources of water in the stem (Tyree \& Yang 1990). The first comes from the xylem itself, which will release water due to cavitation and may  be refilled with water from adjacent living tissue (from the second source). If we know $RWC_{apo}(\Psi_{stem})$, we can calculate the amount (mass) of water per unit of sapwood tissue volume in the stem ($W_{xylem}$ in kgH$_2$O$\cdot$m$^{-3}$):
\begin{equation}
W_{sapwood}(\Psi_{stem}) = RWC_{apo}(\Psi_{stem}) \cdot \rho_{w} \cdot \Theta_{sat}
\end{equation}
where $\rho_{w}$ is the density of water (kgH$_2$O$\cdot$m$^{-3}$) and $\Theta_{sat}$ (m$^3$H$_2$O$\cdot$m$^{-3}$) is sapwood porosity, i.e. the maximum volumetric water content per volume of sapwood. If, in addition, we know $V_{sapwood}$ the volume of sapwood, then the amount of water that is stored in sapwood tissue: 
\begin{equation}
S_{sapwood}(\Psi_{stem}) = V_{sapwood} \cdot W_{sapwood}(\Psi_{stem})
\end{equation}

The second source of water is formed by more elastic living cells in the stem (cambium, phloem and bark cells) and can potentially be a large source of water during relatively high water potentials. This source can be described using the relative water content of the symplasm. This storage compartment has its own water potential and exchanges water with the xylem according to the difference in water potential. The amount of water stored in this compartment at any time is:
\begin{equation}
S_{storage}(\Psi_{storage}) = S_{max, storage} \cdot RWC_{sym}(\Psi_{storage}) \cdot \rho_{w}
\end{equation}
where $S_{max, storage}$ is the maximum storage capacity.
...
\begin{center}
    ...
    L & m·MPa$^{-1}$·s$^{-1}$ & Radial conductivity. \\ \hline
    \end{tabular}
\end{center}
