...
The supply function of the rhizosphere element relates the flow rate to the pressure inside the roots ($\Psi_{root}$). It is calculated by numerical integration of the van Genuchten function (see function \texttt{hydraulics.EVanGenuchten()}), for which we use the analytical approximation of van Lier et al. (2009). Here we draw the supply function for the rhizosphere starting at the four different values of bulk soil pressure ($\Psi_{soil}$) and for the same three texture types:
\begin{center}
<<fig=TRUE, width=4, height=4, echo=FALSE>>==
par(mar=c(4,4,1,1))
...
EVG11 = unlist(lapply(psiVec1, hydraulics.EVanGenuchten, psiSoil1, krmax1, n1, alpha1))
EVG12 = unlist(lapply(psiVec2, hydraulics.EVanGenuchten, psiSoil2, krmax1, n1, alpha1))
EVG13 = unlist(lapply(psiVec3, hydraulics.EVanGenuchten, psiSoil3, krmax1, n1, alpha1))
EVG14 = unlist(lapply(psiVec4, hydraulics.EVanGenuchten, psiSoil4, krmax1, n1, alpha1))
EVG21 = unlist(lapply(psiVec1, hydraulics.EVanGenuchten, psiSoil1, krmax2, n2, alpha2))
EVG22 = unlist(lapply(psiVec2, hydraulics.EVanGenuchten, psiSoil2, krmax2, n2, alpha2))
EVG23 = unlist(lapply(psiVec3, hydraulics.EVanGenuchten, psiSoil3, krmax2, n2, alpha2))
EVG24 = unlist(lapply(psiVec4, hydraulics.EVanGenuchten, psiSoil4, krmax2, n2, alpha2))
EVG31 = unlist(lapply(psiVec1, hydraulics.EVanGenuchten, psiSoil1, krmax3, n3, alpha3))
EVG32 = unlist(lapply(psiVec2, hydraulics.EVanGenuchten, psiSoil2, krmax3, n3, alpha3))
EVG33 = unlist(lapply(psiVec3, hydraulics.EVanGenuchten, psiSoil3, krmax3, n3, alpha3))
EVG34 = unlist(lapply(psiVec4, hydraulics.EVanGenuchten, psiSoil4, krmax3, n3, alpha3))
...
@
\end{center}
The derivative of $dE/d\Psi_{rootcrown}$ for the supply function of the root system is obtained numerically. An advantage of working with elements in parallel is that we can know the water potentials in different points of the root system. Here we plot them for the results of silt loam texture and the first and last soil potential vectors defined above:
\begin{center}
...
<<fig=TRUE, width=8, height=8, echo=FALSE>>=
par(mar=c(4,4,4,1), mfrow=c(2,2))
supplyAG11 = hydraulics.supplyFunctionAboveground(supplyNetwork11$E, supplyNetwork11$psiRoot,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  PLCstem=0,
                                  psiMax = psiMax)
supplyAG11Emb = hydraulics.supplyFunctionAboveground(supplyNetwork11$E, supplyNetwork11$psiRoot,
                                  kstemmax, stemc,stemd,
                                  kleafmax, leafc,leafd,
                                  PLCstem=plcCav,
                                  psiMax = psiMax)
supplyAG11Cap1 = hydraulics.supplyFunctionAbovegroundCap(supplyNetwork11$E, supplyNetwork11$psiRoot,
                                  ...
                                  psiMax = psiMax)
supplyAG11Cap2 = hydraulics.supplyFunctionAbovegroundCap(supplyNetwork11$E, supplyNetwork11$psiRoot,
                                  ...
                                  Vmaxleaf, leaffapo, leafpi0, leafeps,
                                  psiMax = psiMax)
supplyAG11Cap1Emb = hydraulics.supplyFunctionAbovegroundCap(supplyNetwork11$E, supplyNetwork11$psiRoot,
                                  ...
                                  Vmaxleaf, leaffapo, leafpi0, leafeps,
                                  psiMax = psiMax)
supplyAG11Cap2Emb = hydraulics.supplyFunctionAbovegroundCap(supplyNetwork11$E, supplyNetwork11$psiRoot,
                                  ...
                                  Vmaxleaf, leaffapo, leafpi0, leafeps,
                                  psiMax = psiMax)
plot(-supplyAG11$psiLeaf, supplyAG11$E, type="l", col=col1, ylab="Flow rate  (mmol·m-2·s-1)", 
     xlab = "Leaf pressure (-MPa)",  xlim=c(0, 7), ylim=c(-1,6), lwd=1, main="original")
lines(-supplyAG11Cap1$psiLeaf, supplyAG11Cap1$E, lty=1, col=col2)
lines(-supplyAG11Cap2$psiLeaf, supplyAG11Cap2$E, lty=1, col=col3)
legend("bottomright",  legend= c("No capacitance", "Psi_prev = 0 MPa", "Psi_prev = -1.5 MPa"),       col = c(col1, col2, col3), lty=1, bty="n", cex=0.7)
plot(-supplyAG11Emb$psiLeaf, supplyAG11Emb$E, type="l", col=col1, ylab="Flow rate  (mmol·m-2·s-1)", 
     xlab = "Leaf pressure (-MPa)",  xlim=c(0, 7), ylim=c(-1,6), lwd=1, main="cavitated")
legend("bottomright",  legend= c("No capacitance", "Psi_prev = 0 MPa", "Psi_prev = -1.5 MPa"), col = c(col1, col2, col3), lty=1, bty="n", cex=0.7)
lines(-supplyAG11Cap1Emb$psiLeaf, supplyAG11Cap1Emb$E, lty=1, col=col2)
lines(-supplyAG11Cap2Emb$psiLeaf, supplyAG11Cap2Emb$E, lty=1, col=col3)

plot(-supplyAG11$psiLeaf, supplyAG11$dEdP, type="l", col=col1, ylab="dE/dP  (mmol·m-2·s-1·MPa-1)",     xlab = "Leaf pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,max(supplyAG11Cap1$dEdP)), lwd=1, main="original")
legend("topright",  legend= c("No capacitance", "Psi_prev = 0 MPa", "Psi_prev = -1.5 MPa"), col = c(col1, col2, col3), lty=1, bty="n", cex=0.7)
lines(-supplyAG11Cap1$psiLeaf, supplyAG11Cap1$dEdP, lty=1, col=col2)
lines(-supplyAG11Cap2$psiLeaf, supplyAG11Cap2$dEdP, lty=1, col=col3)
plot(-supplyAG11Emb$psiLeaf, supplyAG11Emb$dEdP, type="l", col=col1, ylab="dE/dP  (mmol·m-2·s-1·MPa-1)", 
    xlab = "Leaf pressure (-MPa)",  xlim=c(0, 7), ylim=c(0,max(supplyAG11Cap1$dEdP)), lwd=1, main="cavitated")
legend("topright",  legend= c("No capacitance", "Psi_prev = 0 MPa", "Psi_prev = -1.5 MPa"),col = c(col1, col2, col3), lty=1, bty="n", cex=0.7)
lines(-supplyAG11Cap1Emb$psiLeaf, supplyAG11Cap1Emb$dEdP, lty=1, col=col2)
lines(-supplyAG11Cap2Emb$psiLeaf, supplyAG11Cap2Emb$dEdP, lty=1, col=col3)
@
...
\subsubsection{Effect of water compartments on transpiration}
As mentioned when describing water content functions, we consider two sources of water in plant segments (Tyree \& Yang 1990). Symplastic storage compartments can contribute to sapflow dynamics by exchanging water with the apoplastic compartments, where it is assumed that the bulk of vertical flow occurs. In \texttt{medfate}, there are as stem symplastic storage compartments as stem segments and one leaf symplastic compartment. The instantaneous flow rate between symplastic and apoplastic compartments can be approximated using a lateral conductance $k_{lat}$ and the difference in water potentials. For the leaf we have:
\begin{equation}
...
\item{For every stem segment $i$ and for the leaf segment:}
  \begin{itemize}
    ...
    \item{Subtract the lateral flow from the water content of the symplastic comparment and update its corresponding water potential (i.e. $\Psi_{sym, i, t_m}$ or $\Psi_{sym, l, t_m}$).}
  \end{itemize}
...
\subsubsection{Plant transpiration after disconnecting from the soil}
