...
This is a reference book for simulation and static models included in **medfate**. Hands-on user guides to run simulation functions and static functions can be found as package **vignettes** included in its distribution. Our purpose is to continuously update this reference with package updates, so that users can know about the models they are using at the time when functions are run. As the manual will keep package updates, after model applications users should store a **PDF version** of the reference manual to be sure it matches the version the package reported in the application (e.g. report or article).
...
In this book we use `code` or `variablename` to indicate R code or to refer to variable names within R, and `functionname()` to refer to a package function. When relevant, we will indicate the correspondence between mathematical symbols, their units and variable names within R. 