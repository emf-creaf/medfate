...
In this book we use `code` to indicate R code or to refer to variable names within R. When relevant, we give the correspondence between mathematical symbols and variable names within R. 

The following chapter describes the soil, vegetation and meteorology inputs that apply to all models. After that, the book presents each simulation model using a set of chapters. The first chapter introduces the model and the remaining describe the details of formulations by processes. Those process formulations that are common to more than one model (like hydrology) are described only once.
