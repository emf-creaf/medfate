...
This is a *reference book* for simulation and static models included in **medfate**. Hands-on user guides to run simulation functions and static functions can be found as *package vignettes* within the package. Our aim is to continuously update the reference book along with package developments, so that users have detailed information about the models at the time functions are run. As the manual will follow package updates, after a given model application users should store a **PDF version** of the reference manual to be sure it matches the version the package reported in their application report or article.
