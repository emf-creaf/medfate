...
Eco-hydrological processes are fundamental for the simulation models included in the `medfate` package. Soil water balance can be studied for a single forest stand using function  `spwb()`. Function `spwb()` may be used to:
...
Changes in leaf area and plant growth are key to evaluate the influence of climatic conditions on forest structure and function. Package `medfate` allows simulating plant growth of a set of cohorts (competing for light and water) in a single forest stand using function `growth()`. Function `growth()` may be used to:
...
In this book we use `code` to indicate R code or to refer to variable names within R, and `function()` to refer to a package function. When relevant, we will indicate the correspondence between mathematical symbols, their units and variable names within R. 