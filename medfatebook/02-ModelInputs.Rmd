...
### Soil initialization
Soil initialization is needed to estimate soil hydrological parameters for each soil layer $s$ from their physical attributes. Initialization is done using function `soil()`, which adds the following information to the physical soil description: 
...
Users can also edit the `soil` object manually, for example to provide specific parameters of the Van Genuchten retention curve calibrated from soil samples.
...
| $N$ | $ind · ha^{-1}$ | `N` | The density of tree individuals | N | Y |
