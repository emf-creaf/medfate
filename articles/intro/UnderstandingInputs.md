...
volumetric water content ($\theta$; %) and soil water potential ($\Psi$;
MPa). By default the Saxton (`model = "SX"`) equations are used to model
the water retention curve, but the user may choose to follow Van
Genuchten - Mualem equations, which will give slightly different values
