...
For example, we can use function `forest2aboveground()` on the object `exampleforestMED` to show how medfate completes above-ground information:
```{r}
...
```{r}
above2 = forest2aboveground(exampleforestMED2, SpParamsMED)
above2
```
...
vprofile_leafAreaDensity(exampleforestMED, SpParamsMED, byCohorts = F)
```
```{r, fig = TRUE, fig.width= 5, fig.height=3, fig.align= 'center', echo=TRUE}
vprofile_leafAreaDensity(exampleforestMED, SpParamsMED, byCohorts = T)
```
