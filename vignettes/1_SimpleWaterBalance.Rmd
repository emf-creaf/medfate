...
Most often, users will use function `swb()` to run the soil water balance model. This function requires the `swbInput` object, the `soil` object and the meteorological data frame. However, function `swb.day()` modified the state variables of the input objects. In particular, the values of soil moisture and cumulative growth degree days are now:
```{r}
examplesoil$W
x$canopy$gdd
```
...
x$canopy$gdd
```
Now we are ready to call function `swb()`:
```{r}
S = swb(x, examplesoil, examplemeteo)
```
Function `swb()` returns an object of class with the same name, actually a list:
```{r}
class(S)
```
