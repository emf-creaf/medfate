...
This document describes how to run the growth model of `medfate`, implemented in function `growth()`. All the details of the model design and formulation can be found at the corresponding chapters of the [medfate book](https://emf-creaf.github.io/medfatebook/index.html). 

Because the forest growth model builds on the water balance model, the reader is assumed here to be familiarized with `spwb()`. If not, we recommend reading vignette [*Basic water balance*](https://emf-creaf.github.io/medfate/articles/BasicWaterBalance.html) before this one. 

## Preparing model inputs
Model inputs are explained in greater detail in vignette [*Preparing model inputs*](https://emf-creaf.github.io/medfate/articles/PreparingInputs.html). Here we briefly review the different steps required to run function `growth()`.
...
Weather input needs to be in form of a `data.frame` with days in rows and variables in columns. Here we will also use the default data included in the package:
...
Unless indicated in the control parameters, the `growthInput` object will be modified during simulations. In the case of `growth()`, these modifications concern all structural variables in element `above` as well as other state variables such as carbon storage levels and allocation targets.

Elements starting with `params*` contain cohort-specific model parameters. Some of them were already presented in previous vignettes ([*Basic water balance*](https://emf-creaf.github.io/medfate/articles/BasicWaterBalance.html) and [*Advanced water balance*](https://emf-creaf.github.io/medfate/articles/AdvancedWaterBalance.html)). An important set of new cohort-specific parameters for the forest growth model are `paramsGrowth`:
```{r}
x$paramsGrowth
```
...
As we indicated above, the `growth()` simulations modify (by default) the `growthInput` object. We can check this comparing the initial values in element `above` with the current ones:

```{r}
x$above
```

but elements starting with `internal*`, containing state variables, are also modified. This behaviour is intended to be used to concatenate consecutive calls to `growth()` with the same object.
...
The new list elements, with respect to the output of function `spwb()`, are `LabileCarbonBalance` (components of the labile carbon balance), `PlantBiomassBalance` (plant- and cohort-level biomass balance), `PlantStructure` (daily series of structural variables) and `GrowthMortality` (daily growth and mortality rates).


## Plots and summaries

Users can inspect the output of `growth()` simulations using functions `summary()` and `plot()` on the simulation output. Several plots are available, in addition to all the plots that were available to display the results of `spwb()` simulations. Some of them are illustrated in the following subsections:
...
Evaluation of growth simulations will normally imply the comparison of predicted vs observed basal area increment (BAI) at a given temporal resolution or the comparison of predicted vs observed tree diameter or plant height. 
...
Normally growth evaluations will be at annual scale, but here we only have one year of simulated growth. Assuming we want to evaluate the predictive capacity of the model in terms of monthly basal area increment for the pine cohort, we can plot the relationship using function `evaluation_plot()`:
...
```
And the following would help us quantifying the relationship:
```{r}
evaluation_stats(G1, exampleobs, "BAI", cohort = "T1_148", 
                 temporalResolution = "month")
```

The observed data set is fake and the evaluation is unrealistically good. For illustrative purposes, one could also compare DBH values, here drawing the observed and predicted time series together: 

```{r, echo=TRUE, fig.width=7, fig.height=4}
evaluation_plot(G1, exampleobs, "DBH", cohort = "T1_148", 
