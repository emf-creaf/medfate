...
Regarding **belowground** information, we need a vector with plant rooting depths, which we can build from `Z95` values:
```{r}
Z = c(exampleforestMED$treeData$Z95, exampleforestMED$shrubData$Z95)
Z
```
We also need a matrix describing for each plant cohort, the proportion of fine roots in each soil layer. As before, we use function `forest2belowground()` on the object `exampleforestMED` to show how should the data look like:
```{r}
V = forest2belowground(exampleforestMED, examplesoil, SpParamsMED)
V
```
...
```{r}
x = spwbInput(above, Z, V, examplesoil, SpParamsMED, control)
```
