...
if(!file.exists(file_out)) {
  ...
  for(i in 1:nrow(df_res_growth)) {
    control <- defaultControl(df_res_growth$transpirationMode[i], df_res_growth$soilDomains[i],
                              df_res_growth$rhizosphereOverlap[i])
    control$verbose <- FALSE
  ...
  for(i in 1:nrow(df_res_fordyn)) {
    control <- defaultControl(df_res_fordyn$transpirationMode[i], df_res_fordyn$soilDomains[i],
                              df_res_fordyn$rhizosphereOverlap[i])
    control$verbose <- FALSE
  ...
  names(df_res)[5] <- as.character(medfate_ver)
...
for(i in 1:length(files)) {
  ...
  if(!("rhizosphereOverlap" %in% names(df_res_i))) {
    df_res_i <- df_res_i |>
      dplyr::mutate(rhizosphereOverlap = "total") |>
      dplyr::relocate("rhizosphereOverlap", .after = soilDomains)
  }
  names(df_res_i)[5] <- substr(names(df_res_i)[5], 2, nchar(names(df_res_i)[5]))
  if(is.null(df_res)) {
    df_res <- df_res_i
  } else {
    df_res <- df_res |>
      dplyr::left_join(df_res_i, by = c("model", "transpirationMode", "soilDomains", "rhizosphereOverlap"))
  }
}
df_res |>
