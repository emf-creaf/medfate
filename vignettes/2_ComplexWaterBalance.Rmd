...
output: 
  rmarkdown::html_vignette:
    toc: TRUE
vignette: >
  %\VignetteIndexEntry{2. Complex water balance simulation}
...
This document describes how to run a soil water balance model that uses a rather complex approach for hydraulics and stomatal regulation. This document is meant to teach users to run the simulation model within R. All the details of the model design and formulation are given in a separate document that can be found on the website of the R package (http://vegmod.ctfc.cat/medfateweb). 
...
print(spar)
```
where `widths` are soil layer widths in mm; `clay` and `sand` are the percentage of clay and sand, in percent of dry weight, and `om` stands for organic matter. `macro` specifies the macroporosity of each layer and `rfc` the percentage of rock fragments. Finally, `Gsoil` and `Ksoil` are parameters needed to model water infiltration into the soil. 

### Soil input object
Soil input for water balance models is actually an object of class `soil` that is created using a function with the same name:
```{r}
examplesoil = soil(spar)
```
The soil object contains both soil parameters and state variables. The elements of the soil object are:
```{r}
names(examplesoil)
```
The meaining of all elements in the soil object can be found in the help page for function `soil()`. Importantly, the soil object is used to store the moisture degree of each soil layer. `W` is state variable that represents moisture (i.e. the proportion of moisture relative to field capacity), which is normally initialized to 1 for each layer.
```{r}
examplesoil$W
```
In the case of the complex soil water balance model, another state variable is the temperature of each soil layer. This is stored in element `Temp`, which has missing values by default:
```{r}
examplesoil$Temp
```
It is important to remember that, unlike normal objects in R, any water balance simulation will modify the moisture values in soil object. That is, the state of the soil at the end of the simulated process (i.e. `W` and `Temp`) will be stored. Hence, one can use the same object to simulate water balance sequentially and the final state of one simulation is the initial state of the next.

At any time, one can print the status of the soil object:
```{r}
print(examplesoil, model = "VG")
```
The actual moisture content of the soil depends (mm or litres of water per square meter) on the water retention curve used to represent the relationship between soil volumetric water content ($\theta$; %) and soil water potential ($\Psi$; MPa). As water retention curve, the complex model uses Van Genuchten - Mualem equations. This is why we wrote `model="VG"` above. Functions `soil.psi2thetaVG()` and `soil.theta2psiVG()` can be used to calculate volumetric soil moisture from the water potential, and conversely, according to the Van Genuchten water retention curve.
...
```
If one has a `forest` object, the `swbInput` object can be generated in directly from it, avoiding the need to explicitly build aboveground and belowground data frames:
...
x$above
```
As with the soil input object, the `swbInput` object may be modified during simulations. In the case of soil water balance, these modifications concern `LAI_expanded` and `LAI_dead` in element `above`, as well as canopy variables.
...
### Running the model
Soil water balance simulations will normally span periods of several months or years, but since the model operates at a daily and subdaily temporal scales, it is possible to perform soil water balance for one day only. This is done using function `swb.day()`. In the following code we select day 100 from the meteorological input data and perform soil water balance for that day only:
```{r}
d = 100
...
```
Function `swb.day()` is most useful when working with the complex transpiration model. The output of `swb.day()` is a list with five elements:
```{r}
names(sd1)
```

Running soil water balance model changes the input objects. In particular, the soil moisture at the end of the simulation will be stored in the soil object:
```{r}
examplesoil$W
```
As it is the temperature of soil layers
```{r}
examplesoil$Temp
```
We can also see a change in the state variables of the canopy:
```{r}
x$canopy
```
...
**DailyBalance**: Contains the soil water balance flows of the day (precipitation, infiltration, transpiration, ...)
```{r}
sd1$DailyBalance
```
...
**Plants** contains output values by plant cohort and subdaily time step. Many output variables can be inspected in this element.

```{r}
names(sd1$Plants)
```
With the following code, we use function `plot.swb.day()` to draw plots of daily variation per species of plant transpiration (L·m$^{-2}$), transpiration per leaf area (in L·m$^{-2}$·LAI$^{-1}$), net photosynthesis per leaf area (in g C·m$^{-2}$·LAI$^{-1}$), and plant water potential (in MPa):
```{r, fig=TRUE, fig.align="center", fig.width=8, fig.height = 8}
...
### Running the model
Users will often use function `swb()` to run the soil water balance model for several days. This function requires the `swbInput` object, the `soil` object and the meteorological data frame:
```{r}
S = swb(x, examplesoil, examplemeteo[140:170,], latitude = 41.82592, elevation = 100)
```
...
Package `medfate` also provides a `plot` function for objects of class `swb`. It can be used to show the meteorological input. Additionally, it can also be used to draw soil and plant variables. In the code below we draw water fluxes, soil water potentials, plant transpiration and plant (mid-day) water potential:
...
While the simulation model uses daily steps, users may be interested in outputs at larger time scales. The package provides a `summary` for objects of class `swb`. This function can be used to summarize the model's output at different temporal steps (i.e. weekly, annual, ...). For example, to obtain the average soil moisture and water potentials by months one can use:
