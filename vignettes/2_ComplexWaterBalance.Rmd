...
Calls to `hydraulics.supplyFunctionPlot()` always need both a `swbInput` object and a `soil` object. The soil moisture state (i.e. its water potential) is the starting point for the calculation of the supply function, so different curves will be obtained for different values of soil moisture.
...
```{r, fig=TRUE, fig.align="center", fig.width=7, fig.height = 18}
d = 100
...
The output of `swb.day()` is a list with five elements:
```{r}
names(sd1)
```
