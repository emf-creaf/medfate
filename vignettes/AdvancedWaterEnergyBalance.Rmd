...
```{r, fig=TRUE, fig.align="center", fig.width=7, fig.height = 10}
...
names(sd1$Plants)
```
With the following code, we use function `plot.spwb_day()` to draw plots of daily variation per species of plant transpiration (L·m$^{-2}$), transpiration per leaf area (in L·m$^{-2}$·LAI$^{-1}$), plant net photosynthesis (in g C·m$^{-2}$·LAI$^{-1}$), and plant water potential (in MPa):
