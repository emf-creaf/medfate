...
As with simple water balance, information about the cohort species is found in element `cohorts`  (i.e. code, species and name):
```{r}
x$cohorts
```
...
x$above
```
The `spwbInput` object may be modified during simulations. In the case of soil water balance, these modifications concern `LAI_expanded` and `LAI_dead` in element `above`, as well as canopy variables.
...
The `spwbInput`object includes cohort parameter values for several aspects of plant anatomy and functioning. For example, plan anatomy is described in `paramsAnatomy`.
```{r}
x$paramsAnatomy
```
It also presents parameters related to plant transpiration can be seen in `paramsTransp`:
```{r}
x$paramsTransp
```
and parameters related to water storage of leaf and stem organs:
```{r}
...
```{r, fig=TRUE, fig.align="center", fig.width=4, fig.height = 3.5}
par(mar=c(4,4,1,1))
