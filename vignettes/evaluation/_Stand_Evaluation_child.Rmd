...
```{r, max.print=5}
as.data.frame(df_site$SiteData[[1]]$customParams)|> 
  kbl() |>
  kable_styling()
```
...
<!-- Soil moisture -->
```{r, echo=FALSE, results='asis', warning = FALSE, message=FALSE}
eval_REW <- "SWC" %in% names(df_site$SiteData[[1]]$measuredData)
 if(eval_REW) {
   res <- knitr::knit_child(file.path(this_path,'_REW_Evaluation_child.Rmd'), quiet = TRUE)
   cat(res, sep = '\n')
 }
```
