...
params = list(medfate_ver = medfate_ver,
              wd = wd,
              model='spwb',
              names = c("Puéchabon", "Font-Blanche",
                        "Yatir",
                        "Mitra",
                        "Prades"),
              codes = c("FRAPUE", "FONBLA",
                        "YATIR",
                        "MITRA",
                        "PRADES"),
...
for(i in 1:n_sites) {
  ...
  for(j in 1:n_confs) {
    ...
    df_all$Path[cnt] <- file.path(params$wd,'data', 'output', params$medfate_ver, params$model, site, conf)
    file_name_data <- file.path(params$wd,'data', 'input', site,
                       paste0(site,"_",params$medfate_ver,".rds"))
    ...
    site_list  <- readRDS(file_name_data)
...
```{r}
yaml_path <- "~/OneDrive/mcaceres_work/model_development/medfate_evaluation/StandLevelEvaluation/inst/yaml_configurations/"
control_granier <- yaml::yaml.load_file(paste0(yaml_path, "granier.yaml"))
control_sperry <- yaml::yaml.load_file(paste0(yaml_path, "sperry.yaml"))
control_sureau <- yaml::yaml.load_file(paste0(yaml_path, "sureau.yaml"))
