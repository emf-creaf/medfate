...
df = SpParams[SpParams$SpIndex %in% target_spp, c("Name", "Tmax_LAI", "Tmax_LAIsq", "WUE","Al2As", "SLA", "RGRcambiummax", "fHDmin", "fHDmax")]
row.names(df) = NULL
df %>%
  kbl() %>%
  kable_styling()
...
Predictive capacity by species (for species with > 200 records):
...
bai_error_map(mapdata_barecr)
```


Predictive capacity by species (with ingrowth occurring in more than 50 plots):
