...
for(i in 1:n) {
  ...
  if(is.null(IDs)) IDs <- unique(post$dfbai_plot$id)
  else {
    IDs <- IDs[IDs %in% post$dfbai_plot$id]
  }
...
dfbai_sp <- bind_rows(dfbai_sp_vec) |> filter(id %in% IDs) |> sf::st_as_sf()
dfbai_plot<- bind_rows(dfbai_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfbadead_sp <- bind_rows(dfbadead_sp_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfbadead_plot<- bind_rows(dfbadead_plot_vec) |> filter(id %in% IDs)|> sf::st_as_sf()
dfbarecr_sp <- bind_rows(dfbarecr_sp_vec) |> filter(id %in% IDs)|> sf::st_as_sf()
dfbarecr_plot<- bind_rows(dfbarecr_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfbatot_sp <- bind_rows(dfbatot_sp_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfbatot_plot<- bind_rows(dfbatot_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfNdead_sp <- bind_rows(dfNdead_sp_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfNdead_plot<- bind_rows(dfNdead_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfNrecr_sp <- bind_rows(dfNrecr_sp_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfNrecr_plot<- bind_rows(dfNrecr_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfNtot_sp <- bind_rows(dfNtot_sp_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfNtot_plot<- bind_rows(dfNtot_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfshcov_plot<- bind_rows(dfshcov_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dfshheight_plot<- bind_rows(dfshheight_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dflai_allom_plot<- bind_rows(dflai_allom_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
dflai_state_plot<- bind_rows(dflai_state_plot_vec)|> filter(id %in% IDs) |> sf::st_as_sf()
