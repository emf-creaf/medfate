---
title: "1. Complex water balance simulation"
...
output: 
  rmarkdown::html_vignette:
    toc: TRUE
vignette: >
  %\VignetteIndexEntry{1. Complex water balance simulation}
...
Function `soil.psi2thetaVG()` (and its counterpart) can be used to calculate volumetric soil moisture from the water potential using according to the water retention curve.
...
```
and to use the complex soil water balance model we must change the following option:
...
par(mar=c(4,4,1,1))
plot(sd1, type = "PlantTranspiration")
```
...
```{r}
S = swb(x, examplesoil, examplemeteo[150:170,], latitude = 41.82592, elevation = 100)
```
...
Package `medfate` provides a simple `plot` function for objects of class `swb`. It can be used to show the meteorological input. Additionally, it can also be used to draw plant variables, such as plant transpiration:
