...
df_vpd = readRDS("~/OneDrive/Professional/MedfateWorks/Metamodelling_TR_WUE/Rdata/metamodelling_vpd_results.rds")
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
g3<-ggplot(df_vpd, aes(x = vpd, y = WUEg_rel))+
...
```{r, echo = FALSE}
g4<-ggplot(df_co2, aes(x = Catm, y = Ag_rel))+
  geom_point(aes(col = Name), size=0.3)+
...
g8<-ggplot(df, aes(x=Ag_granier_meta, y = Ag_sperry))+
  ...
  theme_bw()+theme(legend.position = "none")
g0<-plot_grid(g5,g6,g7,g8, ncol=2)
...
#                g3 + theme(legend.position = c(0.73,0.62)),
#                g4 + theme(legend.position = "none"),
#                g7,g8,
#                labels = c("a)","b)", "c)", "d)", "e)","f)"), nrow=3, ncol=2)
# ggsave2("meta_fit_eval.png", g, width = 9, height = 13.5)
```
<!-- ```{r, echo = FALSE} -->
