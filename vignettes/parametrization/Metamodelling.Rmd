...
library(medfate)
library(nlme)
meta_dir <- "~/OneDrive/Professional/MedfateWorks/Metamodelling_TR_WUE/"
```
...
```{r, echo = FALSE}
df = readRDS(paste0(meta_dir,"Rdata/metamodelling_results.rds"))
...
g2<-ggplot(df_K)+
  ...
  xlim(c(-5,0))+
  xlab("Soil water potential (MPa)")+ ylab("E (advanced, drought) / E (advanced, no drought)")+
...
ggsave2("meta_fit.png", g, width = 9, height = 13.5, bg = "white")
```
...
g5<-ggplot(df, aes(x=E_granier, y = E_sperry))+
...
g6<-ggplot(df, aes(x=E_granier_meta, y = E_sperry))+
  geom_point(size=0.3, aes(col=Name), alpha = 0.3)+
  geom_abline(intercept = 0, slope = 1, col="gray", alpha = 0.7, size=2)+
  scale_color_discrete("")+
  ylim(c(0,350))+ xlim(c(0,350))+
...
g7<-ggplot(df, aes(x=Ag_granier, y = Ag_sperry))+
  geom_point(size=0.3, aes(col=Name), alpha = 0.3)+
  geom_abline(intercept = 0, slope = 1, col="gray", alpha = 0.7, size=2)+
  scale_color_discrete("")+
  ...
  theme_bw()+theme(legend.position = "none")+ theme(panel.grid = element_blank())
g8<-ggplot(df, aes(x=Ag_granier_meta, y = Ag_sperry))+
  geom_point(size=0.3, aes(col=Name), alpha = 0.3)+
  geom_abline(intercept = 0, slope = 1, col="gray", alpha = 0.7, size=2)+
  scale_color_discrete("")+
  ylim(c(0,1500))+ xlim(c(0,1500))+
  ...
  theme_bw()+theme(legend.position = "none")+ theme(panel.grid = element_blank())
g0<-plot_grid(g5,g6,g7,g8, labels = c("a)", "b)", "c)", "d)"), 
              ncol=2, nrow = 2)
g<-plot_grid(g0,l, ncol=2, nrow = 1, rel_widths = c(1,0.2))
ggsave2("meta_eval.png", g, width = 11, height = 9, bg = "white")
g
```
