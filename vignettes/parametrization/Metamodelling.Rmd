...
The following parameters are used in the basic model, that cannot easily be parameterized from available information (see https://emf-creaf.github.io/medfatebook/index.html):
 ...
 + `WUE`, which represents the daily water use efficiency (g C of gross assimilation / l H2O transpired) under conditions of high photosynthetically active radiation (PAR), no water deficit and no air CO2 limitations. 
 + `WUE_par`, which is the exponent of the function specifying the decrease in photosynthesis for lower (shadier) positions within the canopy, due to the decrease of available PAR. 
 + `WUE_co2`, which regulates the dependency of gross photosynthesis on air CO2 concentration. 
...
df_co2 = readRDS("~/OneDrive/Professional/MedfateWorks/Metamodelling_TR_WUE/Rdata/metamodelling_co2_results.rds")
...
```{r}
g3<-ggplot(df_co2, aes(x = Catm, y = Ag_rel))+
  geom_point(aes(col = Name), size=0.3)+
...
g <- plot_grid(g1 + theme(legend.position = "none"), 
               ...
               l, labels = c("a)","b)", "c)", ""), nrow=2, ncol=2)
# ggsave2("meta_fit.png", g, width = 9, height = 9)
```
