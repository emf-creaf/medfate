...
This document presents a metamodeling exercise between basic (Granier's) and advanced (Sperry's) versions of the soil plant water balance model. The goal is to make transpiration (E) and gross photosynthesis (Ag) estimates produced by the basic water balance model as similar as possible to those predicted by the advanced model which, given its greater process detail and physical basis, is assumed to provide better E and Ag estimates when provided appropriate functional traits are supplied. 
...
Assuming there are no water limitations, the following parameters are used to determine E and Ag in the basic model, which cannot easily be parameterized from available trait data (see https://emf-creaf.github.io/medfatebook/index.html):
 ...
 + `Tmax_LAI` and `Tmax_LAIsp`, which determine the ratio of maximum transpiration over potential evapotranspiration for a given LAI. An empirical function with these parameters was derived by Granier (1999), but without distinguishing between species. A metamodelling exercise should allow deriving species-specific values for these parameters.
 + `WUE`, which represents the daily water use efficiency (g C of gross assimilation / l transpired) under conditions of high light and no water stress. Such estimate of WUE is not easily available for many species in trait databases, but we can derive it from metamodelling if Farquhar's photosynthetic parameters `Vmax298` and `Jmax298` are known.
...
```{r, echo = FALSE}
data(SpParamsMED)
pardf = data.frame(Name = species_characterParameter(target_spp, SpParamsMED, "Name"),
                   SRL = species_parameter(target_spp, SpParamsMED, "SRL"),
                   FineRootDensity = species_parameter(target_spp, SpParamsMED, "FineRootDensity"),
                   RLD = species_parameter(target_spp, SpParamsMED, "RLD"))
for(i in 1:length(target_spp)) pardf$SRSA[i] = root_specificRootSurfaceArea(pardf$SRL[i], pardf$FineRootDensity[i])
pardf %>%
  kbl() %>%
  kable_styling()
```
...
ggplot(df, aes(x = PAR, y = kplant))+
  geom_point(aes(col = Name), size=0.3, alpha = 0.3)+
  geom_smooth(method = "loess", aes(group = Name, col = Name))+
  ylab("k (plant)")+xlab("PAR (%)")+
  theme_bw()
```

```{r, echo = FALSE, fig = TRUE, message=FALSE}
ggplot(df, aes(x = PAR, y = Ar2Al))+
  geom_point(aes(col = Name), size=0.3, alpha = 0.3)+
  geom_smooth(method = "loess", aes(group = Name, col = Name))+
  ylab("Fine root area / Leaf area")+xlab("PAR (%)")+
  theme_bw()
```
...
TR_sp <- df %>% 
  ...
  summarize(n = n(),
            ...
            kplant_sd = sd(kplant, na.rm=TRUE),
            Ar2Al_mean = mean(Ar2Al, na.rm=TRUE),
            Ar2Al_se = sd(Ar2Al, na.rm=TRUE),
