...
names(df) <-  c("Rel. MAE (%)", "RERsapwood","RGRsapwoodmax", "SRsapwood", "rfc@2")
...
Statistically significant differences can be observed between species for `RERsapwood` and `RGRsapwoodmax`, as shown in the following ANOVA tables:
...
calParamsSpecies[,-1] %>% #Ommits index 
  kbl() %>%
  kable_styling()
```
...
eval_all_df = eval_df[,c("Species", "MAE_all", "MAErel_all","MAE_BAItot_all", "MAErel_BAItot_all")]
names(eval_all_df)<-c("Species", "MAE_BAI", "MAErel_BAI","MAE_BAItot", "MAErel_BAItot")

eval_cal_df = eval_df[,c("Species", "MAE_calibrated", "MAErel_calibrated","MAE_BAItot_calibrated", "MAErel_BAItot_calibrated")]
names(eval_cal_df)<-names(eval_all_df)

eval_spp_df = eval_df[,c("Species", "MAE_spp", "MAErel_spp","MAE_BAItot_spp", "MAErel_BAItot_spp")]
names(eval_spp_df)<-names(eval_all_df)

eval_cal_df$type = "Calibrated"
eval_spp_df$type = "Species-level averages"
eval_all_df$type = "Overall averages"

eval_all_df = bind_rows(eval_cal_df, eval_spp_df, eval_all_df)
eval_all_df$type = factor(eval_all_df$type, levels = c("Calibrated", "Species-level averages","Overall averages"))
```
...
eval_df %>% group_by(Species) %>% 
  summarise(`MAE plot (cm2/yr)` = median(MAE_calibrated, na.rm=T),
            `MAE spp (cm2/yr)` = median(MAE_spp, na.rm=T),
            `MAE all (cm2/yr)` = median(MAE_all, na.rm=T),
            `MAE plot (%)` = median(MAErel_calibrated, na.rm=T),
            `MAE spp (%)` = median(MAErel_spp, na.rm=T),
            `MAE all (%)` = median(MAErel_all, na.rm=T)) %>% 
  kbl() %>%
  kable_styling()
```
...
```{r echo = FALSE, eval = TRUE}
eval_df %>% group_by(Species) %>% 
  summarise(`MAE plot (cm2)` = median(MAE_BAItot_calibrated, na.rm=T),
            `MAE spp (cm2)` = median(MAE_BAItot_spp, na.rm=T),
            `MAE all (cm2)` = median(MAE_BAItot_all, na.rm=T),
            `MAE plot (%)` = median(MAErel_BAItot_calibrated, na.rm=T),
            `MAE spp (%)` = median(MAErel_BAItot_spp, na.rm=T),
            `MAE all (%)` = median(MAErel_BAItot_all, na.rm=T)) %>% 
  kbl() %>%
  kable_styling()
```
