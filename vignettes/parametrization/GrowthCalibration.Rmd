...
eval_def_df$type = "Overall averages"
...
eval_df %>% group_by(Species) %>% 
  summarise(`Observed (cm2/yr)` = median(Obs_BAI, na.rm=T),
            `MAE plot (cm2/yr)` = median(MAE_calibrated, na.rm=T),
            `MAE spp. (cm2/yr)` = median(MAE_calibrated_sp, na.rm=T),
            `MAE all (cm2/yr)` = median(MAE_uncalibrated, na.rm=T),
            `MAE plot (%)` = median(MAErel_calibrated, na.rm=T),
            `MAE spp. (%)` = median(MAErel_calibrated_sp, na.rm=T),
            `MAE all (%)` = median(MAErel_uncalibrated, na.rm=T)) %>% 
  kbl() %>%
  kable_styling()
```
...
```{r echo = FALSE, eval = TRUE}
eval_df %>% group_by(Species) %>% 
  summarise(`Observed (cm2)` = median(Obs_BAItot, na.rm=T),
            `MAE plot (cm2)` = median(MAE_BAItot_calibrated, na.rm=T),
            `MAE spp. (cm2)` = median(MAE_BAItot_calibrated_sp, na.rm=T),
            `MAE all (cm2)` = median(MAE_BAItot_uncalibrated, na.rm=T),
            `MAE plot (%)` = median(MAErel_BAItot_calibrated, na.rm=T),
            `MAE spp. (%)` = median(MAErel_BAItot_calibrated_sp, na.rm=T),
            `MAE all (%)` = median(MAErel_BAItot_uncalibrated, na.rm=T)) %>% 
  kbl() %>%
  kable_styling()
```
