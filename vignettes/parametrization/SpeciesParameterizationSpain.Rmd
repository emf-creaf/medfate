...
Package **medfateutils** is important for the parametrization process, as it includes several functions that facilitate parsing data sources into parameter values. Package **medfateutils** is installed only from GitHub:

```{r, eval = FALSE}
remotes::install_github("emf-creaf/medfateutils")
```

Package **medfuels** is needed because it includes allometric coefficients for Mediterranean shrub species. As before, package **medfuels** is installed only from GitHub:

```{r, eval = FALSE}
remotes::install_github("spif-ctfc/medfuels")
```

Once we have **medfate**, **medfateutils** and **medfuels**, we load them as well as other packages needed for this vignette:
```{r message=FALSE}
library(medfate)
...
```
It is obvious that such a sheer number of parameters cannot be searched for manually, even for a small number of target species. Hence, it is important to draw values for data bases. Not all model parameters have their counterpart in publicly-available databases, and even when using trait data bases it is unlikely that one will find appropriate parameter values for all species. Luckily, **medfate** has inbuilt imputation procedures to estimate missing values for many parameters before starting model simulations. However, imputed parameter estimates will normally be worse than estimates coming from data bases. Hence, we should put a strong effort in finding source parameter data before relying on inbuilt imputation.
...
The following sections describe different steps that we used to obtain the species parameter table `SpParamsMED` for simulations with **medfate**, which can be grouped into five main steps:
...
The first step is to define the target entities that will be considered functionally distinct in **medfate** simulations. There is no *a priori* need to limit such entities to taxons, nor to particular taxonomic ranks. Nevertheless, in this vignette we will define taxonomic entities either at species or genus level.

### Load IFN taxon codes
Model simulations will normally entail a resolution of target entities lower than forest inventory data, because a finer biological resolution results in higher numbers of missing parameter values. Therefore, it will be usual to lump entities treated as separated in the forest inventory. In our case, we begin by loading shrub and tree species codes used in the second, third, and fourth editions of IFN, i.e. IFN2, IFN3 and IFN4:
```{r}
...
```
The final table of species names and codes has `r nrow(IFN_species_data)` and looks like:
```{r}
...
```
so that we are kept with `r nrow(IFN_species_data)` taxa.
...
```
Some codes (particularly, those excluded above) may not be recognized according to the target entity definition of `SpParams` and are ignored. The option `fill_fromGenus = TRUE` is used to tell function `populateGrowthForm()` that all species of the same entity should have the same growth form, so that even if some codes are not mentioned, for whatever reason, growth forms are also defined. Some species may have been defined in the IFN species definition but then be missing from the tree/shrub tables. We identify them:
```{r}
missingGrowthFormSpecies = SpParams[is.na(SpParams$GrowthForm), c("Name")]
print(missingGrowthFormSpecies)
```
and manually set a tree the growth form for them:
```{r}
...
We will first populate allometric coefficients for the relationship between height and projected area. Allometric coefficients are in the following tables for species:
```{r}
data("sp_params_area")
head(sp_params_area)
```
...
Although not included in **medfuels**, we can use the same strategy to populate the fraction of dead material (`pDead` or $P_{dead}$) and the crown ratio (`cr`) for shrubs. First we load the data frame:
```{r}
...
Tree allometric models for foliar biomass were calibrated from data obtained in IEFC, a forest inventory conducted in Catalonia (*Inventari Ecològic i Forestal de Catalunya*) more or less during the same years as IFN3. We begin by loading the table containing regression coefficients:
```{r}
...
Although we used specific functions to map most of the parameters so far, package **medfateutils** offers a generic function, called `populateTraits()`, to help mapping trait values from data bases into model parameters. In this section we will extensively use this function to fill a large number of parameters.

### Trait mapping from TRY

The TRY database is a primary source for plant traits. We created a list of TRY trait definitions that could be mapped to **medfate** parameters. The following table shows the mapping:
...
Additional plant trait databases can be used for parameter estimation. We begin by loading several trait data bases, that drawn from literature publications or assembled by ourselves.
```{r}
#  HYDRATRY
...
```
Leaf shape data is also available from database BROT2. We also standardize BROT2 leaf shape values into the same five categories:
```{r}
...
```
Database BROT2 contains both leaf area measurements and categorical leaf size data. We thus split BROT2 data into leaf area values and leaf size strings:
```{r}
...
```
Leaf phenology types are also available in BROT2, so we select them and map the existing values into **medfate** categories:
```{r}
...
```
Rooting depth values are also available from BROT2 database (some of these records are also in TRY). As before, we assume values to correspond to parameter `Z95`.
```{r}
...
Most parameters regulating leaf phenological phases (i.e. bud dormancy, budburst, senescence, bud formation, ...) require calibration exercises using observed weather data and phenological records. However, here we can map leaf duration from available data bases.
...
```
Leaf lifespan is also available from BROT2, also in number of months:
```{r}
...
```
We also select SLA records from BROT2, already in the same units as TRY.
```{r}
...
```
And we merge these two sources with SLA values coming from database HYDRATRY. The whole set is used to call `populateTraits()` and populate $SLA$ parameter values without any transformation:
```{r}
...
Wood density records are also available from the 'Global Wood Density Database':
```{r}
...
```
Wood density is also part of HYDRATRY database:
```{r}
names(hydratry)[names(hydratry)=="WD"] = "WoodDensity"
```
and we can also find wood density estimates in BROT2:
```{r}
...
```
We tried to reduce some of these missing values with data from BROT2. We first select "DeadFuel" records from this data base:
```{r}
...
Huber values are also available from HIDRATRY, so we transform them into `Al2As`:
```{r}
...
Fuel moisture content is usually defined as the mass of water per dry mass of fuel tissue. We used the database of Yebra et al. (2020) as primary source of fuel moisture content. We first update the name of some species in the data base:
```{r}
...
```
Note that the call to `populateTraits()` includes `erase_previous = FALSE` to avoid overwriting values on those estimated from Yebra et al. (2020):
```{r}
...
Leaf osmotic potential at full turgor (`LeafPI0` or $\pi_{0,leaf}$) and leaf elastic modulus (`LeafEPS` or $\epsilon_{leaf}$) describe leaf pressure-volume curves. These traits can be obtained from TRY and Bartlett et al. (2012). We begin by changing the column names of this second database:
```{r}
...
```
We then fill $\pi_{0,leaf}$ with the TRY and Bartlett et al. (2012) data sources merged:
```{r}
...
Finally, we use Bartlett et al. (2012) to provide suitable values for the fraction of apoplastic tissue (`LeafAF` or $f_{apo,leaf}$):
```{r}
...
Parameter $\Psi_{critic}$ (`Psi_Critic`) is the water potential corresponding to 50\% of loss of hydraulic conductance in the stem, also known as $\Psi_{50}$. This trait can be found in three data bases: 
```{r}
...
Minimum and maximum values of leaf conductance to water vapor ($G_{swmin}$ and $G_{swmax}$) can be drawn from databases of Duursma et al. (2018) and Hoshika et al (2018), respectively. We first populate $G_{swmin}$ using data from Duursma et al. (2018), while chaning the units from $mmol\cdot s^{-1} \cdot m^{-2}$ to $mol\cdot s^{-1} \cdot m^{-2}$:
```{r}
...
```
We then populate $G_{swmax}$ using data from Hoshika et al. (2018) without any change in units:
```{r}
...
Stem maximum hydraulic conductivity ($K_{stem, max, ref}$) is an important parameter used in **medfate** to estimate stem maximum conductance, which defines plant hydraulic efficiency. Here we populate some values from HYDRATRY: 
...
Stem hydraulic vulnerability is described in **medfate** using Weibull-type curves, which are described using two parameters. Trait databases normally report vulnerability using water potential corresponding to different levels of conductance loss, i.e. $\Psi_{12}$, $\Psi_{50}$, $\Psi_{88}$, etc. Here we first estimate Weibull coefficients from Martin-StPaul et al (2017) data on $\Psi_{12}$, $\Psi_{50}$:
```{r}
...
```
and we do something similar from Choat et al (2012) data on $\Psi_{50}$, $\Psi_{88}$:
```{r}
...
```
We then merge them with available data from HYDRATRY and call the population function:
```{r}
...
This document describes the parametrization of bioclimatic recruitment thresholds for the forest dynamics model `fordyn()` included in `medfate`. Specifically, we aim to find suitable values for parameters:
...
Load yearly climatic data (from Santander's Group) between 1981 and 2015, for all IFN plots in Spain:
```{r}
...
If foliar biomass allometries were available for all species in Spain, PAR calculations could include all Spanish forest plots. However, foliar biomass (and hence LAI) values can only be calculated for Catalonia. We hence, load the list of `forest` objects corresponding to IFN3 plots in Catalonia:
```{r, eval = FALSE}
...
```
Since we took Spain for bioclimatic limits and Catalonia for PAR, there are more plots with bioclimatic information (n = `r length(plotBioclim)`) than with PAR information  (n = `r length(plotPAR)`).
...
shrubFiltBioclim <- shrub %>% 
  ...
  filter(Species > 0)
```
We do the same for PAR information, which corresponds to IFN3 plots in Catalonia, only:
```{r}
regFiltPAR <- reg %>% 
...
```
There are `r length(spUnique)` species to be characterized.
...
```{r, include = FALSE}
saveRDS(SpParams, "SpParamsSPAIN_recr.rds")
```
