...
Although not included in **medfuels**, we can use the same strategy to populate parameters `pDead` and `cr` for shrubs, corresponding to the proportion of dead elements and the crown ratio. First we load the data frame:
```{r}
...
### Load trait databases (except TRY)

We begin by loading a several trait data bases, compiled from literature publications or assembled by ourselves.
```{r}
#  HYDRATRY
...
The list does not include the TRY database. In this case we first created a list of TRY trait definitions that could be mapped to **medfate** parameters. We then queried TRY and split the result into one database for each trait, using the TRY trait ID as file name. Data for these plant traits will be progressively loaded as required.
...
A large number of life form records can be found in TRY. We load TRY life form data first: 
```{r}
...
A large number of leaf shape records can be found in TRY (its name in the data base is "Leaf form"). We load TRY leaf shape data first:
```{r}
TRY_leaf_shape  <-readRDS(file.path(DB_path, "TraitDatabases/TRY/TRY_traits/TRY_43.rds"))
```
Since life form descriptions in TRY are not standardized, we standardize life form values into five categories:
```{r}
...
Most parameters regulating leaf phenological phases (i.e. bud dormancy, budburst, senescence, bud formation, ...) require calibration exercises. However, here we can map leaf duration.
...
In this subsection we map several plant anatomy traits into model parameters. All of them are imputable, so that missing values do not need to be handled here.
...
The fraction of standing dead biomass ($P_{dead}$) was already populated from shrubs in the section devoted to allometries:

```{r}
table(is.na(SpParams$pDead))
```
However, we reduce some of these missing values with data from BROT2. We first select "DeadFuel" records from this data base:
```{r}
...
SpParams <- populateTraits(SpParams, duursma, 
                           ...
                           taxon_column = "species",
                           scalar_functions = list("Gswmin" = function(x) {x/1000})) #From mmol·s-1·m-2 to mol·s-1·m-2
