...
Baseline maintenance respiration rates for each tissue (leaf, sapwood and fine roots) are species-specific parameters in the model. However, **medfate** allows estimating these coefficients from nitrogen concentration. Hence it is useful to draw this latter parameter from trait databases. Here we first extract nitrogen concentration per leaf mass values from TRY (TraitID = 14):
```{r photosynthetic_capacity}
TRY_Nleaf <-readRDS(file.path(DB_path,"TraitDatabases/TRY/TRY_traits/TRY_14.rds"))
TRY_Nleaf <- TRY_Nleaf[TRY_Nleaf$ErrorRisk<4, c("AccSpeciesName", "StdValue")]
```
...
TRY_Nsapwood <- TRY_Nsapwood[TRY_Nsapwood$ErrorRisk<4, c("AccSpeciesName", "StdValue")]
TRY_Nfineroot <- TRY_Nfineroot[TRY_Nfineroot$ErrorRisk<4, c("AccSpeciesName", "StdValue")]
