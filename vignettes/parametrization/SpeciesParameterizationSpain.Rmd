...
} else {SpParams = readRDS("SpParamsSPAIN_init.rds")}
```
...
budburst <- readxl::read_excel(fn_pheno, sheet=1)
senescence <- readxl::read_excel(fn_pheno, sheet=2)
```
...
We compiled a data base with published leaf budburst models [@Vitasse2011]:
```{r}
budburst
```
...
senescence
```
We use the leaf budburst model data base to replace the phenology type and leaf phenology parameters:
```{r}
SpParams <- populateTraits(SpParams, as.data.frame(budburst), 
                           trait_mapping = c("PhenologyType" = "PhenologyType"),
                           taxon_column = "Species",
                           character_traits = TRUE,
                           erase_previous = TRUE)
SpParams <- populateTraits(SpParams, as.data.frame(budburst), 
...
```{r}
SpParams <- populateTraits(SpParams, as.data.frame(senescence), 
