...
This document describes how to run the soil water balance model on a forest plot at Font-Blanche (France), using the R function `spwb()` included in package `medfate`. The document indicates how to prepare the model inputs, use the model simulation function and inspect the outputs. 
...
names(fb_SWB)
```
In particular, `WaterBalance` contains water balance components in form of a data frame with days in rows:
```{r}
...
Before examining the results of the model, it is important to compare its predictions against observed data, if available. Evapotranspiration and soil moisture content (0-30 cm layer) was measured at Font-Blanche during the study period of 2014 and this allows comparing the simulation results with observations. 
...
```
Where we see a reasonably good relationship, but the model tends to underestimate total evapotranspiration for low values. We know that interception is missing from modelled ETR, but we cannot discard other sources of error. Function `evaluation_stats()` allows us to generate evaluation statistics:
...
```
Surprisingly, the absolute scales of the modeled and measured SMC do not match, so we can use relative extractable water:
```{r, fig=TRUE, fig.align="center", fig.width=5, fig.height = 4}
...
## Drawing plots
Package `medfate` provides a simple `plot` function for objects of class `spwb`. Here we will use this function to display the seasonal variation predicted by the model, as well as the variation at higher temporal resolution within different selected 3-day periods that we define here:
```{r}
